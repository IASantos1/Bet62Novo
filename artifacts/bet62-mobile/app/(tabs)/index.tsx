      ...
      >
        ...
        >
          <Ionicons
            name="grid-outline"
            size={15}
            color={!selected ? "#f59e0b" : colors.mutedForeground}
          />
          <Text
            style={{
        ...
        {chips.map(({ config }) => {
          ...
          return (
            <Pressable
              key={config.label}
              onPress={() => {
                onSelect(active ? null : config.label);
                Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              }}
              style={({ pressed }) => ({
                flexDirection: "row" as const,
            ...
            >
              {hasErr ? (
                ...
                <Image
                  ...
                  onError={() =>
                    setImgErrors((prev) => new Set([...prev, config.label]))
                  }
                  resizeMode="contain"
                />
              )}
...
const BANNER_CFGS = [
  {
    title: "MÚLTIPLAS",
    subtitle: "POPULARES",
    label: "🔥 COMBOS POPULARES",
    count: "200+ apostas",
  },
  {
    title: "TOP",
    subtitle: "APOSTAS",
    label: "⭐ MAIS APOSTADOS",
    count: "150+ apostas",
  },
  {
    title: "ALTO",
...
function PopularBanners({ matches }: { matches: UpcomingMatch[] }) {
  ...
  const favPool = withOdds
    .filter((m) => m.odds.home >= 1.2 && m.odds.home < 1.5)
    .sort((a, b) => a.odds.home - b.odds.home);
  const medPool = withOdds
    .filter((m) => m.odds.home >= 1.5 && m.odds.home < 2.0)
    .sort((a, b) => a.odds.home - b.odds.home);
  const mid25Pool = withOdds
    .filter((m) => m.odds.home >= 2.0 && m.odds.home < 3.0)
    .sort((a, b) => Math.abs(a.odds.home - 2.5) - Math.abs(b.odds.home - 2.5));
  const high31Pool = withOdds
    .filter((m) => m.odds.home >= 2.8)
    .sort((a, b) => Math.abs(a.odds.home - 3.1) - Math.abs(b.odds.home - 3.1));
    ...
    <View style={{ marginBottom: 4 }}>
      ...
      >
        {chunks.map((events, bi) => {
          ...
          if (!cfg) return null;
          const totalOdds = events
            .reduce((acc, m) => acc * m.odds.home, 1)
            .toFixed(2);
          return (
            <View
              key={bi}
...
function UpcomingCard({ match }: { match: UpcomingMatch }) {
  ...
  const dateStr = isToday
    ...
    : isTomorrow
      ? "Amanhã"
      : kickoffDate.toLocaleDateString("pt-PT", {
          day: "2-digit",
          month: "short",
        });
  const timeStr = kickoffDate.toLocaleTimeString("pt-PT", {
    hour: "2-digit",
    minute: "2-digit",
  });
      ...
      >
        {bannerUrl ? (
          <>
            ...
            <View style={{ padding: 10, paddingTop: 10 }}>{oddsButtons}</View>
          </>
        ) : (
          <View style={{ padding: 12 }}>
            ...
            >
              <MaterialCommunityIcons
                ...
                color={sportCol}
              />
              <Text style={{ fontSize: 14, marginLeft: 4, marginRight: 2 }}>
                {getLeagueFlag(match.league, match.country)}
              </Text>
              <Text
                style={{
...
export default function PreGameScreen() {
  ...
  const sportFiltered = allUpcoming.filter(
    (m) => selectedSport === "all" || m.sport === selectedSport,
  );
  const leagueFiltered = selectedLeague
    ? sportFiltered.filter(
        (m) => findMajorLeague(m.league ?? "")?.label === selectedLeague,
      )
    : sportFiltered;
