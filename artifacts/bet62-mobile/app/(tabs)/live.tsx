    ...
    <View style={{ flexDirection: "row", alignItems: "center", gap: 8 }}>
      ...
      <View style={{ gap: 5 }}>
        <View style={{ flexDirection: "row", gap: 3 }}>
          {sets.map((s, i) => (
            <SetCell key={i} value={s[0]} isCurrent={i === currentIdx} />
          ))}
          {currentPoints != null && (
            <PtCell value={currentPoints[0]} isServing={serving?.[0]} />
          )}
        </View>
        <View style={{ flexDirection: "row", gap: 3 }}>
          {sets.map((s, i) => (
            <SetCell key={i} value={s[1]} isCurrent={i === currentIdx} />
          ))}
          {currentPoints != null && (
            <PtCell value={currentPoints[1]} isServing={serving?.[1]} />
          )}
        </View>
      </View>
    </View>
...
function LiveMatchCard({ match }: { match: LiveMatch }) {
  ...
  const anySusp =
    match.marketSuspension != null &&
    Object.values(match.marketSuspension).some((ts) => ts > now);
  ...
  const bannerUrl = getMatchBannerUrlStable(
    String(match.id),
    match.home,
    match.away,
  );
  ...
  const minuteLabel = (() => {
    ...
    if (isBball) {
      ...
      if (s === "OT") return "OT";
      if (s.startsWith("Q"))
        return `${s}${match._liveExtra?.clockStr ? ` · ${match._liveExtra.clockStr}` : ""}`; // Q1 / Q2 / Q3 / Q4
      return `${match.minute ?? 0}'${match._liveExtra?.clockStr ? ` · ${match._liveExtra.clockStr}` : ""}`;
    }
    if (isHockey) {
      const s = match.status ?? "";
      if (s === "1P" || s === "P1" || s === "1st Period")
        return `1º Per.${match._liveExtra?.clockStr ? ` · ${match._liveExtra.clockStr}` : ""}`;
      if (s === "2P" || s === "P2" || s === "2nd Period")
        return `2º Per.${match._liveExtra?.clockStr ? ` · ${match._liveExtra.clockStr}` : ""}`;
      if (s === "3P" || s === "P3" || s === "3rd Period")
        return `3º Per.${match._liveExtra?.clockStr ? ` · ${match._liveExtra.clockStr}` : ""}`;
      if (s === "OT") return "OT";
      if (s === "SO") return "SO";
    ...
    >
      {isTennis ? (
        <View style={{ flexDirection: "row" as const, gap: 6 }}>
          <OddsButton
            mkt="1x2-home"
            lbl={match.home.split(" ").pop() ?? match.home}
            val={match.odds.home}
          />
          <OddsButton
            mkt="1x2-away"
            lbl={match.away.split(" ").pop() ?? match.away}
            val={match.odds.away}
          />
        </View>
      ) : (
        <View style={{ flexDirection: "row" as const, gap: 6 }}>
          ...
          {hasDraw && (
            <OddsButton mkt="1x2-draw" lbl="Emp." val={match.odds.draw} />
          )}
      ...
      {statusOverlay && (
        <Pressable
          onPress={() => {
            if (isObviousLiveResult && !suspended) {
              Haptics.impactAsync(Haptics.ImpactFeedbackStyle.Light);
              setMarketsOpen(true);
            }
          }}
      ...
      >
        {bannerUrl ? (
          <>
            ...
            <View style={{ padding: 10 }}>{OddsRowContent}</View>
          </>
        ) : (
          <View>
            ...
            <View style={{ padding: 12 }}>
              ...
              >
                ...
                <MaterialCommunityIcons
                  ...
                  color={sportColor}
                />
                <Text style={{ fontSize: 14, marginLeft: 1, marginRight: 2 }}>
...
export default function LiveScreen() {
  ...
  if (actualLive.length > 0) {
    items.push({
      ...
      id: "h-live",
      kind: "live",
      count: actualLive.length,
    });
    actualLive.forEach((m) =>
      items.push({ type: "match", id: m.id, match: m }),
    );
  }
  if (emBreve.length > 0) {
    items.push({
    ...
    <View style={s.container}>
      <View style={s.header}>
        <View style={s.headerRow}>
          ...
          <View style={s.connBadge}>
            ...
            />
            <Text
              style={[s.connText, { color: connected ? "#22c55e" : "#f59e0b" }]}
            >
        ...
        >
          {SPORTS.filter(
            (sp) => sp.key === "all" || presentSports.has(sp.key),
          ).map((sp) => {
            const active = selectedSport === sp.key;
            const col = SPORT_COLORS[sp.key] ?? colors.primary;
            const cnt =
              sp.key === "all"
                ? matches.filter((m) => m.hasRealOdds !== false).length
                : (sportCounts[sp.key] ?? 0);
            return (
              <Pressable
                key={sp.key}
                style={[
      ...
      {lastUpdated === 0 ? (
        ...
        <FlatList
          ...
          contentContainerStyle={{
            paddingTop: 6,
            paddingBottom: Platform.OS === "web" ? 140 : 160,
          }}
          showsVerticalScrollIndicator={false}
          refreshControl={
            <RefreshControl
          ...
          ListEmptyComponent={
            <View style={s.emptyWrap}>
              ...
              <Text style={s.emptySubtext}>
                Nenhum jogo em curso neste momento
              </Text>
            </View>
          }
          renderItem={({ item }) => {
