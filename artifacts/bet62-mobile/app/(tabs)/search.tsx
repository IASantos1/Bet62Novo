  ...
  const allMatches: MatchResult[] = useMemo(() => {
    const live: MatchResult[] = liveMatches
      ...
      .map((m) => ({
        ...
        markets: m.markets,
        marketSuspension: m.marketSuspension,
        date: m.date,
        time: m.time,
      }));
        ...
        <View style={s.matchBody}>
          ...
          <View style={{ flexDirection: "row", alignItems: "center", gap: 4 }}>
            <Text style={{ fontSize: 14 }}>
              {getLeagueFlag(
                match.league,
                (match as { country?: string }).country,
              )}
            </Text>
            <Text style={s.matchLeague} numberOfLines={1}>
          ...
          <View style={s.matchOdds}>
            {(
              [
                ...
                ...(hasDraw
                  ? [{ mkt: "1x2-draw", lbl: "X", val: match.odds.draw }]
                  : []),
                { mkt: "1x2-away", lbl: "2", val: match.odds.away },
              ] as { mkt: string; lbl: string; val: number }[]
            ).map(({ mkt, lbl, val }) => {
                ...
                >
                  ...
                  >
                    <MaterialCommunityIcons
                      name={SPORT_ICONS[sp.key] ?? "trophy"}
                      size={18}
                      color={col}
                    />
                  </View>
            ...
            return (
              ...
              >
                <View style={[s.compIcon, { backgroundColor: col + "20" }]}>
                  <MaterialCommunityIcons
                    name={SPORT_ICONS[comp.sport] ?? "trophy"}
                    size={20}
                    color={col}
                  />
                </View>
