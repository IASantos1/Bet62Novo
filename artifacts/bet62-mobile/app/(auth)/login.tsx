...
export default function LoginScreen() {
  ...
  async function handleLogin() {
    if (!email || !password) {
      Alert.alert("Erro", "Preencha email e palavra-passe.");
      return;
    }
    try {
      setLoading(true);
    ...
    <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === "ios" ? "padding" : undefined}>
      ...
      >
        <View style={s.logoArea}>
          ...
          <Text style={s.headline}>Bem-vindo de volta</Text>
          <Text style={s.subheadline}>Inicia sessão para continuar a apostar</Text>
        </View>
        ...
        <View style={s.footer}>
          <Text style={s.footerText}>Não tens conta?</Text>
          <Pressable onPress={() => router.replace("/(auth)/register")}>
            <Text style={s.footerLink}> Regista-te</Text>
          </Pressable>
        </View>
      </ScrollView>
    </KeyboardAvoidingView>
