...
export function ComprehensiveMarketsSheet({ visible, match, onClose }: Props) {
  ...
  const liveHalf =
    isFootball && isLive && !m?.etExtra && !m?.penExtra
      ? match.status === "HT"
        ? 0
        : (match.minute ?? 0) > 45
          ? 2
  ...
  type V2Lineups = {
    ...
    home: {
      formation?: string;
      starters: V2LineupPlayer[];
      bench: V2LineupPlayer[];
    };
    away: {
      formation?: string;
      starters: V2LineupPlayer[];
  ...
  type V2StandingsResponse = {
    standings: V2StandingRow[];
    groups?: Array<{ name: string; rows: V2StandingRow[] }>;
    league: string;
  };

  const [v2Lineups, setV2Lineups] = useState<V2Lineups | null>(null);
  const [v2LineupsLoading, setV2LineupsLoading] = useState(false);
  const [v2Standings, setV2Standings] = useState<V2StandingsResponse | null>(
    null,
  );
  const [v2StandingsLoading, setV2StandingsLoading] = useState(false);

  const isV2Football =
    match.sport === "football" && String(match.id).startsWith("fb-v2-");
  const rawMatchId = isV2Football
    ? String(match.id).replace("fb-v2-", "")
    : null;
  ...
  useEffect(() => {
    ...
    fetch(`${API_BASE}/matches/stats?${p.toString()}`)
      ...
      .then((d) => {
        if (d) setApiStats(d as ApiMatchStats);
      })
      .catch(() => {})
      .finally(() => setApiStatsLoading(false));
  }, [activeTab, match.id]);

  // Fetch real V2 lineups for football V2 matches (stats tab)
  useEffect(() => {
    if (activeTab !== "stats" || !rawMatchId || v2Lineups || v2LineupsLoading)
      return;
    setV2LineupsLoading(true);
    fetch(`${API_BASE}/matches/v2-lineups?sport=football&matchId=${rawMatchId}`)
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d) setV2Lineups(d as V2Lineups);
      })
      .catch(() => {})
      .finally(() => setV2LineupsLoading(false));
  }, [activeTab, match.id]);

  // Fetch V2 standings for football V2 matches (classificacao tab)
  useEffect(() => {
    if (
      ...
      return;
    setV2StandingsLoading(true);
    fetch(
      `${API_BASE}/matches/v2-standings?sport=football&matchId=${rawMatchId}`,
    )
      .then((r) => (r.ok ? r.json() : null))
      .then((d) => {
        if (d && Array.isArray(d.standings))
          setV2Standings(d as V2StandingsResponse);
      })
      .catch(() => {})
      .finally(() => setV2StandingsLoading(false));
  }, [activeTab, match.id]);
  ...
  function computeStats() {
    ...
    if (m?.bothTeamsScore && m.bothTeamsScore.yes > 1.02) {
      const rY = 1 / m.bothTeamsScore.yes,
        rN = 1 / m.bothTeamsScore.no;
      bttsProb = Math.round((rY / (rY + rN)) * 100);
    }
    let over15: number | null = null,
      over25: number | null = null;
    if (m?.totalGoals) {
      if ((m.totalGoals.over15 ?? 0) > 1.02) {
        const rO = 1 / m.totalGoals.over15,
          rU = 1 / m.totalGoals.under15;
        over15 = Math.round((rO / (rO + rU)) * 100);
      }
      if ((m.totalGoals.over25 ?? 0) > 1.02) {
        const rO = 1 / m.totalGoals.over25,
          rU = 1 / m.totalGoals.under25;
    ...
    if (m?.cards?.o35 && m.cards.o35 > 1.02) {
      const rO = 1 / m.cards.o35,
        rU = 1 / m.cards.u35;
      avgCards =
        Math.round((3.5 + (rO / (rO + rU) > 0.5 ? 0.7 : -0.5)) * 100) / 100;
    }
    let avgCorners: number | null = null;
    if (m?.corners?.o85 && m.corners.o85 > 1.02) {
      const rO = 1 / m.corners.o85,
        rU = 1 / m.corners.u85;
      avgCorners =
        Math.round((8.5 + (rO / (rO + rU) > 0.5 ? 1.5 : -0.8)) * 100) / 100;
    }
    const total = 17;
  ...
  function genPressure(minutes: number) {
    const seed =
      [...match.home].reduce((a, c, i) => a + c.charCodeAt(0) * (i + 1), 0) +
  ...
  const hasReasonBanner =
    isLive &&
    suspReason.length > 0 &&
    (suspReason.includes("VAR") ||
      suspReason.includes("GOLO") ||
      suspReason.includes("GOAL") ||
      suspReason.includes("PENAL") ||
      suspReason.includes("CHANCE") ||
      suspReason.includes("SUSPENSO"));
  ...
  function getSuspText(): string {
    ...
    if (r.includes("PENAL") || r.includes("PÊNALTI") || r.includes("PENÁLT"))
      return "PENÁLTI";
  ...
  const s = StyleSheet.create({
    ...
    dateText: {
      ...
      color: isLive ? "#f59e0b" : colors.mutedForeground,
    },
    teamsRow: {
      flexDirection: "row",
      alignItems: "center",
  ...
  function OddsBtn({
    ...
    if (isSusp) {
      return (
        <View style={[s.oddsBtn, s.oddsBtnLocked]}>
          <Text style={[s.oddsLabel, { color: "#6b7280" }]} numberOfLines={1}>
            {label}
          </Text>
    ...
    if (isObvious) {
      return (
        <View style={[s.oddsBtn, s.oddsBtnObvious]}>
          <Text style={[s.oddsLabel, { color: "#d97706" }]} numberOfLines={1}>
            {label}
          </Text>
          <Text
            style={{
    ...
    return (
      <Pressable
        style={({ pressed }) => [
          s.oddsBtn,
          isSelected ? s.oddsBtnSel : s.oddsBtnNorm,
          { opacity: pressed ? 0.75 : 1 },
        ]}
        onPress={() => handleOdds(market, label, value)}
      >
        <Text
          style={[s.oddsLabel, { color: isSelected ? "#ffffffcc" : "#9ca3af" }]}
          numberOfLines={1}
        >
          {label}
        </Text>
  ...
  function StatsContent() {
    ...
    return (
      <View style={{ paddingHorizontal: 14, paddingTop: 14 }}>
          ...
          <View style={s.statRow}>
            <View style={s.statCol}>
              ...
              <Text style={s.statValue}>
                {a ? a.avgStats.goalsScored.toFixed(2) : st.eg.toFixed(2)}
              </Text>
              <Text style={s.statSubtitle}>
                Liga: {a ? a.avgStats.leagueGoals.toFixed(2) : "2.55"}
              </Text>
            </View>
            <View style={s.statCol}>
              <Text style={s.statLabel}>AEM</Text>
              <Text style={s.statValue}>
                {a ? `${a.avgStats.btts}%` : `${st.bttsProb ?? 46}%`}
              </Text>
              <Text style={s.statSubtitle}>
                Liga: {a ? `${a.avgStats.leagueBtts}%` : "46%"}
              </Text>
            </View>
          </View>
          {isFootball && (
            <View style={s.statRow}>
              <View style={s.statCol}>
                ...
                <Text style={s.statValue}>
                  {a ? `${a.avgStats.over15}%` : `${st.over15 ?? 79}%`}
                </Text>
                <Text style={s.statSubtitle}>
                  Liga: {a ? `${a.avgStats.leagueOver15}%` : "79%"}
                </Text>
              </View>
              <View style={s.statCol}>
                <Text style={s.statLabel}>Mais de 2.5</Text>
                <Text style={s.statValue}>
                  {a ? `${a.avgStats.over25}%` : `${st.over25 ?? 50}%`}
                </Text>
                <Text style={s.statSubtitle}>
                  Liga: {a ? `${a.avgStats.leagueOver25}%` : "50%"}
                </Text>
              </View>
            </View>
          )}
          {isFootball && (
            <View style={[s.statRow, { marginBottom: 0 }]}>
              <View style={s.statCol}>
                ...
                <Text style={s.statValue}>
                  {a
                    ? a.avgStats.cards.toFixed(2)
                    : (st.avgCards ?? 3.8).toFixed(2)}
                </Text>
              </View>
              <View style={s.statCol}>
                ...
                <Text style={s.statValue}>
                  {a
                    ? a.avgStats.corners.toFixed(2)
                    : (st.avgCorners ?? 9.5).toFixed(2)}
                </Text>
              </View>
            </View>
          )}
        ...
        <View style={s.statCard}>
          ...
          <View style={s.winCountRow}>
            <View style={{ alignItems: "center" }}>
              ...
              <Text style={s.winCountLabel}>
                Vitórias {match.home.split(" ")[0]}
              </Text>
            </View>
            {probD > 0 && (
              <View style={{ alignItems: "center" }}>
                <Text style={[s.winCountNum, { color: "#eab308" }]}>
                  {h2hD}
                </Text>
              ...
              return (
                <View
                  ...
                  style={{
                    ...
                    alignItems: "center",
                    paddingVertical: 7,
                    borderBottomWidth: i < a.h2hMatches.length - 1 ? 1 : 0,
                    borderBottomColor: colors.border,
                  }}
                >
                  <Text
                  ...
                  <Text
                    style={{
                      ...
                      fontSize: 11,
                      fontFamily: "Inter_600SemiBold",
                      color: isHomeWin ? "#3b82f6" : colors.foreground,
                      textAlign: "right",
                    }}
                    numberOfLines={1}
                  >
                  ...
                  >
                    ...
                    <View
                      style={{
                        backgroundColor: isAwayWin ? "#3b1a1a" : "#3f3f46",
                        borderRadius: 4,
                        paddingHorizontal: 7,
                        paddingVertical: 3,
                      }}
                  ...
                  <Text
                    style={{
                      ...
                      fontSize: 11,
                      fontFamily: "Inter_600SemiBold",
                      color: isAwayWin ? "#ef4444" : colors.foreground,
                    }}
                    numberOfLines={1}
                  >
                    {m2.away}
        ...
        {/* Real V2 lineups (football only) */}
        {isV2Football && v2LineupsLoading && (
          <View
            style={[s.statCard, { alignItems: "center", paddingVertical: 20 }]}
          >
            <Text
              style={{
                fontSize: 12,
        ...
        {isV2Football &&
          !v2LineupsLoading &&
          v2Lineups &&
          v2Lineups.home.starters.length > 0 && (
            <View style={s.statCard}>
              <View
                style={{
                  flexDirection: "row",
              ...
              <View style={{ flexDirection: "row", gap: 12 }}>
                <View style={{ flex: 1 }}>
                  ...
                  {v2Lineups.home.starters.map((p, i) => (
                    <View
                      key={i}
                      style={{
                        flexDirection: "row",
                        ...
                        paddingVertical: 4,
                        borderBottomWidth:
                          i < v2Lineups.home.starters.length - 1 ? 1 : 0,
                ...
                <View style={{ flex: 1 }}>
                  ...
                  {v2Lineups.away.starters.map((p, i) => (
                    <View
                      key={i}
                      style={{
                        flexDirection: "row",
                        ...
                        paddingVertical: 4,
                        borderBottomWidth:
                          i < v2Lineups.away.starters.length - 1 ? 1 : 0,
                        borderBottomColor: "#27272a",
                      }}
                    >
                      <Text
      ...
      <View style={{ paddingHorizontal: 14, paddingTop: 14 }}>
        ...
        >
          ...
          <View style={{ flex: 1 }}>
            ...
            >
              Campo animado com dados em tempo real
            </Text>
          </View>
          <Ionicons
              ...
              >
                ...
                >
                  <View
                    style={{
                      width: "70%",
                      height: Math.max(3, Math.round((pt.home / 11) * HALF)),
                      backgroundColor: "#ef4444",
                      borderRadius: 2,
                    }}
                  />
                ...
                >
                  <View
                    style={{
                      width: "70%",
                      height: Math.max(3, Math.round((pt.away / 11) * HALF)),
                      backgroundColor: "#22c55e",
                      borderRadius: 2,
                    }}
                  />
  ...
  function ClassificacaoContent() {
    const isLoading =
      (isV2Football ? v2StandingsLoading : apiStatsLoading) &&
      !(isV2Football ? v2Standings : apiStats);
    if (isLoading) {
      return (
        <View style={{ paddingTop: 60, alignItems: "center" }}>
          <Ionicons
    ...
    const StandingsTableRows = ({
      ...
      <>
        {rows.map((t, i) => {
          const isMatch =
            t.name.toLowerCase().includes(homeNorm.split(" ")[0]!) ||
            t.name.toLowerCase().includes(awayNorm.split(" ")[0]!);
          return (
            <View
              key={i}
              style={{
                ...
                alignItems: "center",
                paddingVertical: 5,
                backgroundColor: isMatch ? "#1e293b" : "transparent",
                borderRadius: isMatch ? 6 : 0,
                paddingHorizontal: isMatch ? 4 : 0,
                marginHorizontal: isMatch ? -4 : 0,
              }}
            >
              <Text
                style={{
              ...
              <Text
                style={{
                  flex: 1,
                  fontSize: 11,
                  fontFamily: isMatch ? "Inter_700Bold" : "Inter_400Regular",
                  color: isMatch ? colors.primary : colors.foreground,
                }}
                numberOfLines={1}
              >
                {t.name}
    ...
    // V2 standings (real API data — for V2 football matches)
    if (isV2Football && v2Standings && v2Standings.standings.length > 0) {
      const groups = v2Standings.groups;
      const leagueTitle = v2Standings.league || match.league || "Classificação";
      return (
        <View style={{ paddingHorizontal: 14, paddingTop: 14 }}>
          <View style={s.statCard}>
            <View
            ...
            >
              ...
              <Text style={s.statSectionTitle}>{leagueTitle}</Text>
            </View>
            {groups && groups.length > 0 ? (
              groups.map((group, gi) => (
                <View
                  key={gi}
                  style={{ marginBottom: gi < groups.length - 1 ? 16 : 0 }}
                >
                  <View
                    style={{
                      backgroundColor: "#27272a",
              ...
              <>
                <TableHeader />
                <StandingsTableRows
                  rows={v2Standings.standings.slice(0, 20)}
                  totalRows={v2Standings.standings.length}
                />
              </>
            )}
          </View>
    ...
    // Fallback: generated standings from /stats
    const st = apiStats?.standings;
    const teams = st?.teams ?? [];
    return (
      <View style={{ paddingHorizontal: 14, paddingTop: 14 }}>
        {teams.length === 0 && !apiStatsLoading ? (
          <View
            style={[s.statCard, { alignItems: "center", paddingVertical: 24 }]}
          >
            <Text
              style={{
                fontSize: 13,
          ...
          <View style={s.statCard}>
            ...
            >
              <Ionicons name="trophy-outline" size={14} color="#f59e0b" />
              <Text style={s.statSectionTitle}>
                {st?.league ?? match.league ?? "Classificação"}
              </Text>
            </View>
            <TableHeader />
            <StandingsTableRows
              rows={teams.slice(0, 16)}
          ...
          <View style={s.header}>
            ...
            <View style={s.leagueRow}>
              <Text style={{ fontSize: 16, marginRight: 5 }}>
                {getLeagueFlag(match.league, match.country)}
              </Text>
            ...
            <View style={s.teamsRow}>
              ...
              >
                ...
                <Pressable
                  onPress={() => setActiveTab("classificacao")}
                  hitSlop={{ top: 6, bottom: 6, left: 6, right: 6 }}
                >
                  <Ionicons
                    name="trophy-outline"
              ...
              <Text
                style={[s.teamText, { textAlign: "right" }]}
                numberOfLines={1}
              >
                {match.away}
              </Text>
            ...
            {showSuspBanner && (
              <View style={isBigChance ? s.bigChanceBanner : s.suspBanner}>
                <Ionicons
                  name={isBigChance ? "flash" : "warning"}
                  size={14}
                  color={isBigChance ? "#f59e0b" : "#ef4444"}
                />
                <Text
                  style={{
                    fontSize: 11,
            ...
            {activeTab !== "aovivo" &&
              ...
              activeTab !== "classificacao" && (
                <>
                    ...
                    <Section title="Resultado Final" tabKey="resultado">
                      <View style={s.row}>
                        <OddsBtn
                          market="1x2-home"
                          label={
                            isFootball || isHockey ? "Casa (1)" : match.home
                          }
                        ...
                        <OddsBtn
                          market="1x2-away"
                          label={
                            isFootball || isHockey ? "Fora (2)" : match.away
                          }
                  ...
                  {isFootball &&
                    m?.doubleChance &&
                    m.doubleChance.homeOrDraw > 1.01 && (
                      <Section title="Dupla Chance" tabKey="dupla">
                        <View style={s.row}>
                          <OddsBtn
                            market="dc-home-draw"
                            label="1 ou X"
                            value={m.doubleChance.homeOrDraw}
                          />
                          <OddsBtn
                            market="dc-away-draw"
                            label="X ou 2"
                            value={m.doubleChance.awayOrDraw}
                          />
                          <OddsBtn
                            market="dc-home-away"
                            label="1 ou 2"
                            value={m.doubleChance.homeOrAway}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isFootball &&
                    m?.bothTeamsScore &&
                    m.bothTeamsScore.yes > 1.01 && (
                      <Section title="Ambas as Equipas Marcam" tabKey="dupla">
                        <View style={s.row}>
                          <OddsBtn
                            market="btts-yes"
                            label="Sim"
                            value={m.bothTeamsScore.yes}
                          />
                          <OddsBtn
                            market="btts-no"
                            label="Não"
                            value={m.bothTeamsScore.no}
                          />
                        </View>
                      </Section>
                    )}
                    ...
                    [
                      ...
                      <Section title="Total de Golos" tabKey="gols">
                        {[
                          {
                            line: "0.5",
                            o: m.totalGoals.over05,
                            u: m.totalGoals.under05,
                            ko: "tg-o05",
                            ku: "tg-u05",
                          },
                          {
                            line: "1.5",
                            o: m.totalGoals.over15,
                            u: m.totalGoals.under15,
                            ko: "tg-o15",
                            ku: "tg-u15",
                          },
                          {
                            line: "2.5",
                            o: m.totalGoals.over25,
                            u: m.totalGoals.under25,
                            ko: "tg-o25",
                            ku: "tg-u25",
                          },
                          {
                            line: "3.5",
                            o: m.totalGoals.over35,
                            u: m.totalGoals.under35,
                            ko: "tg-o35",
                            ku: "tg-u35",
                          },
                          {
                            line: "4.5",
                            o: m.totalGoals.over45,
                            u: m.totalGoals.under45,
                            ko: "tg-o45",
                            ku: "tg-u45",
                          },
                          {
                            line: "5.5",
                            o: m.totalGoals.over55,
                            u: m.totalGoals.under55,
                            ko: "tg-o55",
                            ku: "tg-u55",
                          },
                          {
                            line: "6.5",
                            o: m.totalGoals.over65,
                            u: m.totalGoals.under65,
                            ko: "tg-o65",
                            ku: "tg-u65",
                          },
                        ]
                    ...
                    <Section title="Total Golos — Par / Ímpar" tabKey="gols">
                      <View style={s.row}>
                        <OddsBtn
                          market="godd"
                          label="Ímpar"
                          value={m.goalOddEven.odd}
                        />
                        <OddsBtn
                          market="geven"
                          label="Par"
                          value={m.goalOddEven.even}
                        />
                      </View>
                    </Section>
                  )}
                  ...
                  {isFootball &&
                    (m as any)?.correctScore &&
                    Object.keys(
                      (m as any).correctScore as Record<string, number>,
                    ).length > 0 && (
                    ...
                    <Section title="Golos Exatos" tabKey="placar">
                      <View style={s.row}>
                        <OddsBtn
                          market="eg-0"
                          label="0 Golos"
                          value={m.exactGoals.g0}
                        />
                        <OddsBtn
                          market="eg-1"
                          label="1 Golo"
                          value={m.exactGoals.g1}
                        />
                        <OddsBtn
                          market="eg-2"
                          label="2 Golos"
                          value={m.exactGoals.g2}
                        />
                      </View>
                      <View style={s.row}>
                        <OddsBtn
                          market="eg-3"
                          label="3 Golos"
                          value={m.exactGoals.g3}
                        />
                        <OddsBtn
                          market="eg-4"
                          label="4 Golos"
                          value={m.exactGoals.g4}
                        />
                        <OddsBtn
                          market="eg-5plus"
                          label="5+ Golos"
                          value={m.exactGoals.g5plus}
                        />
                      </View>
                    </Section>
                  )}
                  ...
                  {isFootball &&
                    m?.teamGoals &&
                    [
                      m.teamGoals.homeOver05,
                      m.teamGoals.homeOver15,
                      m.teamGoals.homeOver25,
                    ].some((v) => (v ?? 0) > 1.01) && (
                      <Section title={`Golos — ${match.home}`} tabKey="gols">
                        {[
                          {
                            line: "0.5",
                            o: m.teamGoals.homeOver05,
                            u: m.teamGoals.homeUnder05,
                            ko: "hg-o05",
                            ku: "hg-u05",
                          },
                          {
                            line: "1.5",
                            o: m.teamGoals.homeOver15,
                            u: m.teamGoals.homeUnder15,
                            ko: "hg-o15",
                            ku: "hg-u15",
                          },
                          {
                            line: "2.5",
                            o: m.teamGoals.homeOver25,
                            u: m.teamGoals.homeUnder25,
                            ko: "hg-o25",
                            ku: "hg-u25",
                          },
                        ]
                  ...
                  {isFootball &&
                    m?.teamGoals &&
                    [
                      m.teamGoals.awayOver05,
                      m.teamGoals.awayOver15,
                      m.teamGoals.awayOver25,
                    ].some((v) => (v ?? 0) > 1.01) && (
                      <Section title={`Golos — ${match.away}`} tabKey="gols">
                        {[
                          {
                            line: "0.5",
                            o: m.teamGoals.awayOver05,
                            u: m.teamGoals.awayUnder05,
                            ko: "ag-o05",
                            ku: "ag-u05",
                          },
                          {
                            line: "1.5",
                            o: m.teamGoals.awayOver15,
                            u: m.teamGoals.awayUnder15,
                            ko: "ag-o15",
                            ku: "ag-u15",
                          },
                          {
                            line: "2.5",
                            o: m.teamGoals.awayOver25,
                            u: m.teamGoals.awayUnder25,
                            ko: "ag-o25",
                            ku: "ag-u25",
                          },
                        ]
                  ...
                  {isFootball && m?.winToNil && m.winToNil.home > 1.01 && (
                    <Section
                      title="Vitória a Zeros (Win to Nil)"
                      tabKey="especiais"
                    >
                      <View style={s.row}>
                        <OddsBtn
                          market="wtn-home"
                          label={match.home}
                          value={m.winToNil.home}
                        />
                        <OddsBtn
                          market="wtn-away"
                          label={match.away}
                          value={m.winToNil.away}
                        />
                      </View>
                    </Section>
                  )}
                  ...
                  {isFootball &&
                    m?.toWinBothHalves &&
                    m.toWinBothHalves.home > 1.01 && (
                      <Section title="Vencer os Dois Tempos" tabKey="especiais">
                        <View style={s.row}>
                          <OddsBtn
                            market="wbh-home"
                            label={match.home}
                            value={m.toWinBothHalves.home}
                          />
                          <OddsBtn
                            market="wbh-away"
                            label={match.away}
                            value={m.toWinBothHalves.away}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isFootball &&
                    m?.highestScoringHalf &&
                    m.highestScoringHalf.first > 1.01 && (
                      <Section title="Tempo com Mais Golos" tabKey="especiais">
                        <View style={s.row}>
                          <OddsBtn
                            market="hsh-first"
                            label="1º Tempo"
                            value={m.highestScoringHalf.first}
                          />
                          <OddsBtn
                            market="hsh-second"
                            label="2º Tempo"
                            value={m.highestScoringHalf.second}
                          />
                          <OddsBtn
                            market="hsh-equal"
                            label="Igual"
                            value={m.highestScoringHalf.equal}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isFootball &&
                    m?.handicap &&
                    m.handicap.homeMinusOne > 1.01 && (
                      <Section title="Handicap Europeu" tabKey="handicap">
                        <View style={s.row}>
                          <OddsBtn
                            market="hcp-home1"
                            label={`${match.home} -1`}
                            value={m.handicap.homeMinusOne}
                          />
                          <OddsBtn
                            market="hcp-away1"
                            label={`${match.away} +1`}
                            value={m.handicap.awayPlusOne}
                          />
                        </View>
                        {m.handicap.homeMinusOneHalf > 1.01 && (
                          <View style={s.row}>
                            <OddsBtn
                              market="hcp-home15"
                              label={`${match.home} -1.5`}
                              value={m.handicap.homeMinusOneHalf}
                            />
                            <OddsBtn
                              market="hcp-away15"
                              label={`${match.away} +1.5`}
                              value={m.handicap.awayPlusOneHalf}
                            />
                          </View>
                        )}
                      </Section>
                  ...
                  {!isFootball &&
                    m?.handicapPoints &&
                    m.handicapPoints.home > 1.01 && (
                      <Section title="Handicap" tabKey="handicap">
                        <View style={s.row}>
                          <OddsBtn
                            market="hcp-home"
                            label={`${match.home} ${m.handicapPoints.line > 0 ? "+" : ""}${m.handicapPoints.line}`}
                            value={m.handicapPoints.home}
                          />
                          <OddsBtn
                            market="hcp-away"
                            label={`${match.away} ${m.handicapPoints.line > 0 ? "-" : "+"}${Math.abs(m.handicapPoints.line)}`}
                            value={m.handicapPoints.away}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isFootball &&
                    show1tempo &&
                    m?.halfTime &&
                    m.halfTime.home > 1.01 && (
                      <Section title="1º Tempo — Resultado" tabKey="1tempo">
                        <View style={s.row}>
                          <OddsBtn
                            market="ht-home"
                            label="Casa (1)"
                            value={m.halfTime.home}
                          />
                          {m.halfTime.draw > 1.01 && (
                            <OddsBtn
                              market="ht-draw"
                              label="Empate (X)"
                              value={m.halfTime.draw}
                            />
                          )}
                          <OddsBtn
                            market="ht-away"
                            label="Fora (2)"
                            value={m.halfTime.away}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isFootball &&
                    show1tempo &&
                    m?.firstGoal &&
                    m.firstGoal.home > 1.01 && (
                      <Section title="1º Golo" tabKey="1tempo">
                        <View style={s.row}>
                          <OddsBtn
                            market="fg-home"
                            label={match.home}
                            value={m.firstGoal.home}
                          />
                          <OddsBtn
                            market="fg-none"
                            label="Sem Golos"
                            value={m.firstGoal.noGoal}
                          />
                          <OddsBtn
                            market="fg-away"
                            label={match.away}
                            value={m.firstGoal.away}
                          />
                        </View>
                      </Section>
                    )}
                    ...
                    <Section title="2º Tempo — Resultado" tabKey="2tempo">
                      <View style={s.row}>
                        <OddsBtn
                          market="sh-home"
                          label="Casa (1)"
                          value={m.secondHalf.home}
                        />
                        {m.secondHalf.draw > 1.01 && (
                          <OddsBtn
                            market="sh-draw"
                            label="Empate (X)"
                            value={m.secondHalf.draw}
                          />
                        )}
                        <OddsBtn
                          market="sh-away"
                          label="Fora (2)"
                          value={m.secondHalf.away}
                        />
                      </View>
                    </Section>
                  )}
                  ...
                  {isFootball &&
                    m?.htft &&
                    m.htft.hh > 1.01 &&
                    (activeTab === "todos" || activeTab === "htft") && (
                      <View
                        style={{
                          paddingHorizontal: 14,
                          paddingTop: 12,
                      ...
                      >
                        ...
                        {[
                          [
                            { k: "htft-hh", l: "1/1", v: m.htft.hh },
                            { k: "htft-hd", l: "1/X", v: m.htft.hd },
                            { k: "htft-ha", l: "1/2", v: m.htft.ha },
                          ],
                          [
                            { k: "htft-dh", l: "X/1", v: m.htft.dh },
                            { k: "htft-dd", l: "X/X", v: m.htft.dd },
                            { k: "htft-da", l: "X/2", v: m.htft.da },
                          ],
                          [
                            { k: "htft-ah", l: "2/1", v: m.htft.ah },
                            { k: "htft-ad", l: "2/X", v: m.htft.ad },
                            { k: "htft-aa", l: "2/2", v: m.htft.aa },
                          ],
                        ].map((row, ri) => (
                      ...
                      {[
                        ...
                        .map((row) => (
                          <View key={row.label} style={s.row}>
                            <OddsBtn
                              market={row.ko}
                              label={`Mais ${row.label}`}
                              value={row.o}
                            />
                            <OddsBtn
                              market={row.ku}
                              label={`Menos ${row.label}`}
                              value={row.u}
                            />
                          </View>
                        ))}
                  ...
                  {isFootball && showET && m?.etExtra && (
                    <>
                      {m.etExtra.tieWinner.home > 1.01 && (
                        <Section
                          title="Vencedor da Eliminatória"
                          tabKey="prolongamento"
                        >
                          <View style={s.row}>
                            <OddsBtn
                              market="et-tw-home"
                              label={match.home}
                              value={m.etExtra.tieWinner.home}
                            />
                            <OddsBtn
                              market="et-tw-away"
                              label={match.away}
                              value={m.etExtra.tieWinner.away}
                            />
                          </View>
                        </Section>
                      )}
                      {m.etExtra.etResult.home > 1.01 && (
                        <Section
                          title="Resultado da Prorrogação"
                          tabKey="prolongamento"
                        >
                          <View style={s.row}>
                            <OddsBtn
                              market="et-home"
                              label="Casa"
                              value={m.etExtra.etResult.home}
                            />
                            <OddsBtn
                              market="et-draw"
                              label="Pen."
                              value={m.etExtra.etResult.draw}
                            />
                            <OddsBtn
                              market="et-away"
                              label="Fora"
                              value={m.etExtra.etResult.away}
                            />
                          </View>
                        </Section>
                      )}
                      {m.etExtra.totalGoals.o05 > 1.01 && (
                        <Section
                          title="Golos na Prorrogação"
                          tabKey="prolongamento"
                        >
                          {[
                            {
                              line: "0.5",
                              o: m.etExtra.totalGoals.o05,
                              u: m.etExtra.totalGoals.u05,
                              ko: "et-o05",
                              ku: "et-u05",
                            },
                            {
                              line: "1.5",
                              o: m.etExtra.totalGoals.o15,
                              u: m.etExtra.totalGoals.u15,
                              ko: "et-o15",
                              ku: "et-u15",
                            },
                            {
                              line: "2.5",
                              o: m.etExtra.totalGoals.o25,
                              u: m.etExtra.totalGoals.u25,
                              ko: "et-o25",
                              ku: "et-u25",
                            },
                          ]
                        ...
                        <Section title="Próximo Golo" tabKey="prolongamento">
                          <View style={s.row}>
                            <OddsBtn
                              market="et-ng-home"
                              label={match.home}
                              value={m.etExtra.nextGoal.home}
                            />
                            <OddsBtn
                              market="et-ng-away"
                              label={match.away}
                              value={m.etExtra.nextGoal.away}
                            />
                          </View>
                        </Section>
                      )}
                  ...
                  {isFootball &&
                    showPen &&
                    m?.penExtra &&
                    m.penExtra.winner.home > 1.01 && (
                      <Section title="Vencedor nos Penáltis" tabKey="penaltis">
                        <View style={s.row}>
                          <OddsBtn
                            market="pen-home"
                            label={match.home}
                            value={m.penExtra.winner.home}
                          />
                          <OddsBtn
                            market="pen-away"
                            label={match.away}
                            value={m.penExtra.winner.away}
                          />
                        </View>
                      </Section>
                    )}
                  ...
                  {isBball &&
                    m?.totalGoals &&
                    m._total != null &&
                    m.totalGoals.over25 > 1.01 && (
                      <Section title="Total de Pontos" tabKey="pontos">
                        {[
                          {
                            line: String(m._total ?? "215.5"),
                            o: m.totalGoals.over25,
                            u: m.totalGoals.under25,
                            ko: `b-pts-o-${String(m._total ?? "215.5")}`,
                            ku: `b-pts-u-${String(m._total ?? "215.5")}`,
                          },
                        ]
                          .filter((r) => r.o > 1.01)
                          .map((row) => (
                  ...
                  {isBball &&
                    (m as any)?.basketballExtra?.totalsRange?.length > 0 && (
                      <Section title="Linhas de Totais" tabKey="pontos">
                        {(
                          (m as any).basketballExtra.totalsRange as {
                            line: number;
                            over: number;
                            under: number;
                          }[]
                          ...
                          .filter(
                            (tr) => tr.line !== ((m as any)?._total ?? -1),
                          )
                          .map((tr) => (
                            <View key={`tr-${tr.line}`} style={s.row}>
                              <OddsBtn
                                market={`b-pts-o-${tr.line}`}
                                label={`Mais ${tr.line}`}
                                value={tr.over}
                              />
                              <OddsBtn
                                market={`b-pts-u-${tr.line}`}
                                label={`Menos ${tr.line}`}
                                value={tr.under}
                              />
                            </View>
                  ...
                  {isHockey &&
                    (m as any)?.hockeyExtra?.period2Total?.over > 1.01 && (
                      <Section
                        title={`Total 2º Período — O/U ${(m as any).hockeyExtra.period2Total.line}`}
                        tabKey="periodos"
                      >
                        <View style={s.row}>
                          <OddsBtn
                            market={`p2t-o-${(m as any).hockeyExtra.period2Total.line}`}
                            label={`Mais ${(m as any).hockeyExtra.period2Total.line}`}
                            value={(m as any).hockeyExtra.period2Total.over}
                          />
                          <OddsBtn
                            market={`p2t-u-${(m as any).hockeyExtra.period2Total.line}`}
                            label={`Menos ${(m as any).hockeyExtra.period2Total.line}`}
                            value={(m as any).hockeyExtra.period2Total.under}
                          />
                        </View>
                      </Section>
                    )}
                  {isHockey &&
                    (m as any)?.hockeyExtra?.period3Total?.over > 1.01 && (
                      <Section
                        title={`Total 3º Período — O/U ${(m as any).hockeyExtra.period3Total.line}`}
                        tabKey="periodos"
                      >
                        <View style={s.row}>
                          <OddsBtn
                            market={`p3t-o-${(m as any).hockeyExtra.period3Total.line}`}
                            label={`Mais ${(m as any).hockeyExtra.period3Total.line}`}
                            value={(m as any).hockeyExtra.period3Total.over}
                          />
                          <OddsBtn
                            market={`p3t-u-${(m as any).hockeyExtra.period3Total.line}`}
                            label={`Menos ${(m as any).hockeyExtra.period3Total.line}`}
                            value={(m as any).hockeyExtra.period3Total.under}
                          />
                        </View>
                      </Section>
                    )}
                    ...
                    <Section title="1º Quarto" tabKey="1periodo">
                      <View style={s.row}>
                        <OddsBtn
                          market="q1-home"
                          label={match.home}
                          value={(m as any).basketballExtra.q1.home}
                        />
                        <OddsBtn
                          market="q1-away"
                          label={match.away}
                          value={(m as any).basketballExtra.q1.away}
                        />
                      </View>
                    </Section>
                  )}
                  ...
                  {isTennis &&
                    m?.tennisExtra &&
                    (activeTab === "todos" || activeTab === "sets") && (
                      <View
                        style={{
                          paddingHorizontal: 14,
                          paddingTop: 12,
                      ...
                      >
                        {(() => {
                          ...
                          const showSet3 =
                            currentSetNum >= 3 && (set3?.home ?? 0) > 1.01;
                          const showTotalSets =
                            currentSetNum <= 2 &&
                            (m.tennisExtra!.totalSets?.over15 ?? 0) > 1.01;
                          const ses = m.tennisExtra!.setExactScore;
                          const sesEntries = ses
                            ? Object.entries(ses).filter(
                                ([, v]) => (v as number) > 1.01,
                              )
                            : [];
                          return (
                                ...
                                <>
                                  ...
                                  <View style={s.row}>
                                    <OddsBtn
                                      market="s1-home"
                                      label={match.home}
                                      value={m.tennisExtra!.firstSet.home}
                                    />
                                    <OddsBtn
                                      market="s1-away"
                                      label={match.away}
                                      value={m.tennisExtra!.firstSet.away}
                                    />
                                  </View>
                                </>
                              )}
                                ...
                                <>
                                  ...
                                  <View style={s.row}>
                                    <OddsBtn
                                      market="s2-home"
                                      label={match.home}
                                      value={m.tennisExtra!.set2.home}
                                    />
                                    <OddsBtn
                                      market="s2-away"
                                      label={match.away}
                                      value={m.tennisExtra!.set2.away}
                                    />
                                  </View>
                                </>
                              )}
                                ...
                                <>
                                  ...
                                  <View style={s.row}>
                                    <OddsBtn
                                      market="ts-o15"
                                      label="Mais 1.5 sets"
                                      value={m.tennisExtra!.totalSets!.over15}
                                    />
                                    <OddsBtn
                                      market="ts-u15"
                                      label="Menos 1.5 sets"
                                      value={m.tennisExtra!.totalSets!.under15}
                                    />
                                  </View>
                                </>
                              )}
                                ...
                                <>
                                  ...
                                  <View style={[s.row, { flexWrap: "wrap" }]}>
                                    {sesEntries.map(([score, odd]) => (
                                      <OddsBtn
                                        key={`ses-${currentSetNum}-${score}`}
                                        market={`ses${currentSetNum}-${score}`}
                                        label={score}
                                        value={odd as number}
                                      />
                                    ))}
                  ...
                  {match.sport === "baseball" &&
                    m?.totalGoals &&
                    (m as any)?._total != null &&
                    m.totalGoals.over25 > 1.01 && (
                      <Section title="Total de Corridas" tabKey="pontos">
                        {[
                          {
                            line: String((m as any)._total - 0.5),
                            o: m.totalGoals.over25,
                            u: m.totalGoals.under25,
                            ko: `mlb-tot-o-${String((m as any)._total - 0.5)}`,
                            ku: `mlb-tot-u-${String((m as any)._total - 0.5)}`,
                          },
                          {
                            line: String((m as any)._total),
                            o: m.totalGoals.over35,
                            u: m.totalGoals.under35,
                            ko: `mlb-tot-o-${String((m as any)._total)}`,
                            ku: `mlb-tot-u-${String((m as any)._total)}`,
                          },
                          {
                            line: String((m as any)._total + 0.5),
                            o: m.totalGoals.over45,
                  ...
                  {/* Baseball F5 */}
                  {match.sport === "baseball" &&
                    (m as any)?.mlbExtra?.f5Result?.home > 1.01 && (
                      <Section title="Primeiras 5 Entradas" tabKey="pontos">
                        <View style={s.row}>
                          <OddsBtn
                            market="mlb-f5-home"
                            label={match.home}
                            value={(m as any).mlbExtra.f5Result.home}
                          />
                          <OddsBtn
                            market="mlb-f5-away"
                            label={match.away}
                            value={(m as any).mlbExtra.f5Result.away}
                          />
                        </View>
                        {(m as any).mlbExtra?.f5Total?.over > 1.01 && (
                          <View style={s.row}>
                            <OddsBtn
                              market={`mlb-f5t-o-${(m as any).mlbExtra.f5Total.line}`}
                              label={`Mais ${(m as any).mlbExtra.f5Total.line}`}
                              value={(m as any).mlbExtra.f5Total.over}
                            />
                            <OddsBtn
                              market={`mlb-f5t-u-${(m as any).mlbExtra.f5Total.line}`}
                              label={`Menos ${(m as any).mlbExtra.f5Total.line}`}
                              value={(m as any).mlbExtra.f5Total.under}
                            />
                          </View>
                        )}
                      </Section>
                  ...
                  {match.sport === "baseball" &&
                    ...
                    m.handicap.homeMinusOne > 1.01 && (
                      <Section title="Run Line (±1.5)" tabKey="runline">
                        <View style={s.row}>
                          <OddsBtn
                            market="mlb-rl-home-1.5"
                            label={`${match.home} -1.5`}
                            value={m.handicap.homeMinusOne}
                          />
                          <OddsBtn
                            market="mlb-rl-away-1.5"
                            label={`${match.away} +1.5`}
                            value={m.handicap.awayPlusOne}
                          />
                        </View>
