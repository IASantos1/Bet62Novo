...
//
// "suspend_event" is the deliberate, explicit exception (user, 2026-08-11,
// a follow-up request to give the Odds/Risk agents real execution
// authority instead of only proposing) — see executor.ts's
// AUTO_EXECUTE_PAIRS. It can only ever act in the SAFE direction: turn
// betting OFF on an event (forceSuspend=true in event_admin_overrides, the
// same lever routes/adminPro.ts already exposes to a human admin) — it
// never unsuspends, never changes odds/margin/RTP (no such lever exists in
// this schema to hand it), and an admin can always reverse it from the
// existing admin-pro panel exactly like a manual suspension.
