...
function basketballLeagueAllowed(
  countryRaw: string,
  leagueName: string,
): boolean {
  const countryKey = normalizeCountryKey(countryRaw);
...
function makeAdvancedMarketsReal(
  ...
  const bttsType = types.find((t) =>
    t.name.toLowerCase().includes("both teams"),
  );
  if (bttsType) {
    const bk = Array.isArray(bttsType.bookmaker)
      ? bttsType.bookmaker[0]
      : bttsType.bookmaker;
    if (bk?.odd) {
  ...
  if (ouType) {
    const bk = Array.isArray(ouType.bookmaker)
      ? ouType.bookmaker[0]
      : ouType.bookmaker;
    if (bk?.total) {
      ...
      if (line15) {
        ...
        if (over > 0 && under > 0) {
          base.totalGoals.over15 = r(over);
          base.totalGoals.under15 = r(under);
        }
      }
      ...
      if (line35) {
        ...
        if (over > 0 && under > 0) {
          base.totalGoals.over35 = r(over);
          base.totalGoals.under35 = r(under);
        }
      }
    }
  }
...
function makeVolleyballMarketsFromTeams(
  home: string,
  away: string,
  realHomeOdds?: number,
  realAwayOdds?: number,
  ...
  const p4s =
    3 * Math.pow(pSetH, 3) * (1 - pSetH) + 3 * Math.pow(1 - pSetH, 3) * pSetH;
  const p5s = Math.max(0, 1 - p3s - p4s);
  const pMatchH = mc(
    Math.pow(pSetH, 3) +
      3 * Math.pow(pSetH, 3) * (1 - pSetH) +
      6 * Math.pow(pSetH, 3) * Math.pow(1 - pSetH, 2) * 0.5,
    0.1,
    0.9,
  );
  ...
  const [oSets35, uSets35] = probsToDecimalOdds(
    [mc(p5s, 0.1, 0.6), mc(1 - p5s, 0.4, 0.9)],
    1.06,
  );
  const pS1H = mc(pSetH + (sr(7) - 0.5) * 0.08, 0.15, 0.85);
  const pS2H = mc(pSetH + (sr(8) - 0.5) * 0.09, 0.15, 0.85);
  const pS3H = mc(0.5 + (pSetH - 0.5) * 0.55 + (sr(9) - 0.5) * 0.1, 0.15, 0.85);
  ...
  const [hcapH, hcapA] = probsToDecimalOdds(
    [
      mc(p3s * (pSetH / (pSetH + (1 - pSetH))), 0.05, 0.85),
      mc(1 - p3s * (pSetH / (pSetH + (1 - pSetH))), 0.15, 0.95),
    ],
    1.06,
  );
  ...
  const pPtsHcapHome = mc(
    1 - normalCdf((-Math.abs(ptsDiffLine) - ptsDiffMean) / ptsDiffSd),
    0.05,
    0.95,
  );
  const [ptsHcapH, ptsHcapA] = probsToDecimalOdds(
    [pPtsHcapHome, 1 - pPtsHcapHome],
    1.06,
  );
  return {
    doubleChance: { homeOrDraw: 0, awayOrDraw: 0, homeOrAway: 0 },
...
function isTennisElite(ev: SAPIV2Event): boolean {
  ...
  const homeName =
    typeof ev.homeTeam === "object"
      ? (ev.homeTeam.name ?? "")
      : (ev.homeTeam ?? "");
  const awayName =
    typeof ev.awayTeam === "object"
      ? (ev.awayTeam.name ?? "")
      : (ev.awayTeam ?? "");
...
async function getFootballV2RedCards(
  matchId: number,
): Promise<V2RedCards | null> {
  const cached = v2FootballRedCardsCache.get(matchId);
  if (cached && Date.now() - cached.fetchedAt < V2_RED_CARDS_TTL_MS)
    return { home: cached.home, away: cached.away };
  try {
    const resp = await fetch(
      `${SAPI_V2_FOOTBALL}/match/${matchId}/statistics`,
      {
    ...
    const redCount = (side: "home" | "away"): number => {
      const ev = get(data, ["event_summary"]) as
        ...
        | undefined;
      const s =
        (ev?.[side] as Record<string, unknown> | undefined) ?? undefined;
      const r = toArr(
...
function enrichTennisV1League(compName: string): string {
  ...
  for (const [k, v] of _tennisV2LeagueLabelByCompName) {
    ...
    if (
      kStripped !== k &&
      (norm.includes(kStripped) || kStripped.includes(norm))
    )
      return v;
  }
  ...
  const cityPart = (norm.split(" - ")[0] ?? norm)
    .trim()
    .replace(/'/g, "")
    .replace(/\s+/g, " ");
...
const dailyFutureCache = new Map<
  number,
  { data: SAPILeagueV2[]; fetchedAt: number }
>();
...
type TennisStatsPlayerRaw = {
  ...
  stats?: {
    period?: Array<{
      ...
      type?: Array<{
        name: string;
        stat?: Array<{ name: string; value: string }>;
      }>;
    }>;
  };
};
type TennisStatsTournament = {
  ...
  name: string;
  match:
    | { id: string; player: TennisStatsPlayerRaw | TennisStatsPlayerRaw[] }
    | Array<{
        id: string;
    ...
    for (const league of leagues) {
      ...
      for (const m of matches) {
        ...
        const rec = {
          ...
          away,
          htHome:
            typeof m.ht?.home_goals === "number" ? m.ht.home_goals : undefined,
          htAway:
            typeof m.ht?.away_goals === "number" ? m.ht.away_goals : undefined,
          homeTeam: m.home.name,
          awayTeam: m.away.name,
          status: m.status,
          finishedAt: Date.now(),
...
async function getDailyLeaguesForFutureOffset(
  offset: number,
): Promise<SAPILeagueV2[]> {
...
function scaleAdvancedMarkets(
  m: AdvancedMarkets,
  factor: number,
): AdvancedMarkets {
  const s = (n: number) =>
    n <= 0 ? n : Math.round(Math.max(1.01, n * factor) * 100) / 100;
  return {
    ...m,
    doubleChance: {
      homeOrDraw: s(m.doubleChance.homeOrDraw),
      awayOrDraw: s(m.doubleChance.awayOrDraw),
      homeOrAway: s(m.doubleChance.homeOrAway),
    },
    bothTeamsScore: {
      yes: s(m.bothTeamsScore.yes),
      no: s(m.bothTeamsScore.no),
    },
    totalGoals: {
      over05: s(m.totalGoals.over05),
      under05: s(m.totalGoals.under05),
      over15: s(m.totalGoals.over15),
      under15: s(m.totalGoals.under15),
      over25: s(m.totalGoals.over25),
      under25: s(m.totalGoals.under25),
      over35: s(m.totalGoals.over35),
      under35: s(m.totalGoals.under35),
      over45: s(m.totalGoals.over45),
      under45: s(m.totalGoals.under45),
      over55: s(m.totalGoals.over55),
      under55: s(m.totalGoals.under55),
      over65: s(m.totalGoals.over65),
      under65: s(m.totalGoals.under65),
    },
    handicap: {
      homeMinusOne: s(m.handicap.homeMinusOne),
      awayPlusOne: s(m.handicap.awayPlusOne),
      homeMinusOneHalf: s(m.handicap.homeMinusOneHalf),
      awayPlusOneHalf: s(m.handicap.awayPlusOneHalf),
    },
    halfTime: m.halfTime
      ? {
          home: s(m.halfTime.home),
          draw: s(m.halfTime.draw),
          away: s(m.halfTime.away),
        }
      : m.halfTime,
    firstGoal: m.firstGoal
      ? {
          home: s(m.firstGoal.home),
          noGoal: s(m.firstGoal.noGoal),
          away: s(m.firstGoal.away),
        }
      : m.firstGoal,
    drawNoBet: m.drawNoBet
      ? { home: s(m.drawNoBet.home), away: s(m.drawNoBet.away) }
      : undefined,
    asianHandicap: m.asianHandicap
      ? {
          line: m.asianHandicap.line,
          home: s(m.asianHandicap.home),
          away: s(m.asianHandicap.away),
        }
      : undefined,
    correctScore: m.correctScore
      ? Object.fromEntries(
          Object.entries(m.correctScore).map(([k, v]) => [k, s(v)]),
        )
      : undefined,
    corners: m.corners
      ? {
          ...
          o95: s(m.corners.o95),
          u95: s(m.corners.u95),
          o105: s(m.corners.o105),
          u105: s(m.corners.u105),
        }
      : undefined,
    cards: m.cards
      ? {
...
function buildNHLLiveMatches(tournaments: NHLTournament[]): LiveMatchState[] {
  ...
  const sorted = [...tournaments].sort(
    (a, b) => hockeyLeaguePriority(a.league) - hockeyLeaguePriority(b.league),
  );
    ...
    for (const m of matches) {
      ...
      if (diff !== 0 && isLive) {
        ...
        liveOdds =
          diff > 0
            ? {
                home: Math.max(1.04, +(odds.home * (1 - factor)).toFixed(2)),
                draw: 0,
                away: Math.min(12, +(odds.away * (1 + factor)).toFixed(2)),
              }
            : {
                home: Math.min(12, +(odds.home * (1 + factor)).toFixed(2)),
                draw: 0,
                away: Math.max(1.04, +(odds.away * (1 - factor)).toFixed(2)),
              };
      }
      liveOdds.draw = 0;
  ...
  for (const t of tournaments) {
    ...
    for (const m of matches) {
      ...
      if (!isLive && m.status === "Not Started") {
        ...
        const dateOk =
          matchDate === todayStr ||
          (allowYesterday && matchDate === yesterdayStr);
        if (dateOk) {
          try {
            let gameMs: number;
            if (m.datetime_utc) {
              // Statpal datetime_utc format: "DD.MM.YYYY HH:MM" (actual UTC)
              const parts = m.datetime_utc.match(
                /^(\d{2})\.(\d{2})\.(\d{4})\s+(\d{2}):(\d{2})/,
              );
              if (parts) {
                const [, dd, mon, yyyy, hh, min] = parts;
                gameMs = new Date(
                  `${yyyy}-${mon}-${dd}T${hh}:${min}:00Z`,
                ).getTime();
              } else {
      ...
      for (const key of inKeys) {
        const h = parseInt(m.home[key]);
        const a = parseInt(m.away[key]);
      ...
      const baseOdds =
        realEntry && realEntry.h > 1 && realEntry.a > 1
      ...
      if (diff !== 0 && isLive) {
        const factor = Math.min(0.4, Math.abs(diff) * 0.1);
        liveOdds =
          diff > 0
            ? {
                home: Math.max(
                  1.04,
                  +(baseOdds.home * (1 - factor)).toFixed(2),
                ),
                draw: 0,
                away: Math.min(12, +(baseOdds.away * (1 + factor)).toFixed(2)),
              }
            : {
                home: Math.min(12, +(baseOdds.home * (1 + factor)).toFixed(2)),
                draw: 0,
                away: Math.max(
                  1.04,
                  +(baseOdds.away * (1 - factor)).toFixed(2),
                ),
              };
      }
        ...
        _liveExtra:
          innings.length > 0
            ? {
                innings,
                outs:
                  m.outs !== undefined && m.outs !== ""
                    ? parseInt(m.outs)
                    : undefined,
                homeHits:
                  parseInt(m.home.hits) > 0 ? parseInt(m.home.hits) : undefined,
                awayHits:
                  parseInt(m.away.hits) > 0 ? parseInt(m.away.hits) : undefined,
                homeErrors:
                  parseInt(m.home.errors) > 0
                    ? parseInt(m.home.errors)
                    : undefined,
                awayErrors:
                  parseInt(m.away.errors) > 0
                    ? parseInt(m.away.errors)
                    : undefined,
              }
            : undefined,
      });
...
function buildNBALiveMatches(tournaments: NBATournament[]): LiveMatchState[] {
  const NBA_LIVE_STATUSES = new Set([
    "Q1",
    "Q2",
    "Q3",
    "Q4",
  ...
  const sorted = [...tournaments].sort(
    (a, b) =>
      basketballLeaguePriority(a.league) - basketballLeaguePriority(b.league),
  );
  ...
  for (const t of sorted) {
    ...
    for (const m of matches) {
      ...
      if (isNotStarted && matchStartsInMinutes(m.date ?? "", m.time) > 90)
        continue;
      ...
      const q1 = [
        parseInt(m.home.q1 ?? "0") || 0,
        parseInt(m.away.q1 ?? "0") || 0,
      ] as [number, number];
      const q2 = [
        parseInt(m.home.q2 ?? "0") || 0,
        parseInt(m.away.q2 ?? "0") || 0,
      ] as [number, number];
      const q3 = [
        parseInt(m.home.q3 ?? "0") || 0,
        parseInt(m.away.q3 ?? "0") || 0,
      ] as [number, number];
      const q4 = [
        parseInt(m.home.q4 ?? "0") || 0,
        parseInt(m.away.q4 ?? "0") || 0,
      ] as [number, number];
      const ot = [
        parseInt(m.home.ot ?? "0") || 0,
        parseInt(m.away.ot ?? "0") || 0,
      ] as [number, number];
      ...
      const qNum =
        st.includes("1") || st === "Q1"
          ? 1
          : st.includes("2") || st === "Q2" || st === "HT" || st === "Halftime"
            ? 2
            : st.includes("3") || st === "Q3"
              ? 3
              : st.includes("4") || st === "Q4"
      ...
      if (diff !== 0 && isLive) {
        const factor = Math.min(0.4, Math.abs(diff) * 0.03);
        liveOdds =
          diff > 0
            ? {
                home: Math.max(1.04, +(odds.home * (1 - factor)).toFixed(2)),
      ...
      const leagueName =
        t.league.includes("Nba") || t.league.includes("NBA") ? "NBA" : t.league;
...
function v1GameToV2Event(g: V1LiveGame): SAPIV2Event {
  return {
    ...
    status: g.statusText ?? "",
    startTimestamp: g.startTime
      ? Math.floor(new Date(g.startTime).getTime() / 1000)
      : undefined,
...
async function fetchLiveRace(
  ...
  const tryV2 = async (): Promise<SAPIV2Event[]> => {
    const resp = await fetch(`${v2Base}/live`, {
      signal: AbortSignal.timeout(3000),
      headers,
    });
...
async function getFootballLiveV2(): Promise<SAPIV2Event[]> {
  const now = Date.now();
  if (
    (wsConnected.has("football") || v1WsConnected.has("football")) &&
    footballLiveV2Cache &&
    now - footballLiveV2FetchedAt < WS_PREFERRED_MAX_STALE_MS &&
    (wsLastMessageAt.get("football") ?? 0) > now - WS_PREFERRED_MAX_STALE_MS
  )
...
async function getBasketballLiveV2(): Promise<SAPIV2Event[]> {
  ...
  if (
    ...
    now - basketballLiveV2FetchedAt < WS_PREFERRED_MAX_STALE_MS &&
    (wsLastMessageAt.get("basketball") ?? 0) > now - WS_PREFERRED_MAX_STALE_MS
  )
    return basketballLiveV2Cache;
  if (
    basketballLiveV2Cache &&
    now - basketballLiveV2FetchedAt < CONFIG.LIVE_CACHE_TTL
  )
    return basketballLiveV2Cache;
  try {
    ...
    if (
      basketballLiveV2Cache &&
      now - basketballLiveV2FetchedAt > V2_LIVE_MAX_STALE_MS
    ) {
      basketballLiveV2Cache = [];
      basketballLiveV2FetchedAt = now;
      return [];
...
async function getHockeyLiveV2(): Promise<SAPIV2Event[]> {
  ...
  if (
    ...
    hockeyLiveV2Cache &&
    now - hockeyLiveV2FetchedAt < WS_PREFERRED_MAX_STALE_MS &&
    (wsLastMessageAt.get("hockey") ?? 0) > now - WS_PREFERRED_MAX_STALE_MS
  )
  ...
  try {
    ...
    if (
      hockeyLiveV2Cache &&
      now - hockeyLiveV2FetchedAt > V2_LIVE_MAX_STALE_MS
    ) {
      hockeyLiveV2Cache = [];
      hockeyLiveV2FetchedAt = now;
      return [];
...
async function getBaseballLiveV2(): Promise<SAPIV2Event[]> {
  ...
  if (
    ...
    baseballLiveV2Cache &&
    now - baseballLiveV2FetchedAt < WS_PREFERRED_MAX_STALE_MS &&
    (wsLastMessageAt.get("baseball") ?? 0) > now - WS_PREFERRED_MAX_STALE_MS
  )
    return baseballLiveV2Cache;
  if (
    baseballLiveV2Cache &&
    now - baseballLiveV2FetchedAt < CONFIG.LIVE_CACHE_TTL
  )
    return baseballLiveV2Cache;
  try {
    ...
    if (
      baseballLiveV2Cache &&
      now - baseballLiveV2FetchedAt > V2_LIVE_MAX_STALE_MS
    ) {
      baseballLiveV2Cache = [];
      baseballLiveV2FetchedAt = now;
      return [];
...
async function getTennisLiveV2(): Promise<SAPIV2Event[]> {
  ...
  if (
    ...
    tennisLiveV2Cache &&
    now - tennisLiveV2FetchedAt < WS_PREFERRED_MAX_STALE_MS &&
    (wsLastMessageAt.get("tennis") ?? 0) > now - WS_PREFERRED_MAX_STALE_MS
  )
    return tennisLiveV2Cache;
  if (tennisLiveV2Cache && now - tennisLiveV2FetchedAt < CONFIG.LIVE_CACHE_TTL)
    return tennisLiveV2Cache;
  try {
    // Race V1 vs V2 — tennis endpoint shape varies, handle both formats
    const headers = sapiHeaders();
    const tryTennis = async (base: string): Promise<SAPIV2Event[]> => {
      const resp = await fetch(`${base}/live`, {
        signal: AbortSignal.timeout(3000),
        headers,
      });
    ...
    const events = await Promise.any([
      tryTennis(SAPI_V1_TENNIS),
      tryTennis(SAPI_V2_TENNIS),
    ]).catch(() => []);
    if (events.length > 0) {
    ...
    if (
      tennisLiveV2Cache &&
      now - tennisLiveV2FetchedAt > V2_LIVE_MAX_STALE_MS
    ) {
      tennisLiveV2Cache = [];
      tennisLiveV2FetchedAt = now;
      return [];
...
function liveMatchIdForSportV2(sport: SportKey, id: number | string): string {
  switch (sport) {
    case "football":
      return `football-v2-${id}`;
    case "basketball":
      return `bball-v2-${id}`;
    case "hockey":
      return `hockey-v2-${id}`;
    case "baseball":
      return `baseball-v2-${id}`;
    case "tennis":
      return `tennis-v2-${id}`;
  }
}

function mergeIncomingPreferFastScore(
  sport: SportKey,
  incoming: SAPIV2Event[],
  existing: SAPIV2Event[] | null,
  now: number,
): SAPIV2Event[] {
...
function applyV2WsMessage(sport: SportKey, raw: unknown): void {
  ...
  switch (sport) {
    case "football":
      ...
      footballLiveV2Cache = mergeIncomingPreferFastScore(
        sport,
        incoming,
        footballLiveV2Cache,
        now,
    ...
    case "basketball":
      ...
      basketballLiveV2Cache = mergeIncomingPreferFastScore(
        sport,
        incoming,
        basketballLiveV2Cache,
        now,
    ...
    case "hockey":
      ...
      hockeyLiveV2Cache = mergeIncomingPreferFastScore(
        sport,
        incoming,
        hockeyLiveV2Cache,
        now,
    ...
    case "baseball":
      ...
      baseballLiveV2Cache = mergeIncomingPreferFastScore(
        sport,
        incoming,
        baseballLiveV2Cache,
        now,
    ...
    case "tennis":
      ...
      tennisLiveV2Cache = mergeIncomingPreferFastScore(
        sport,
        incoming,
        tennisLiveV2Cache,
        now,
...
function mergeSingleV2Event(sport: SportKey, ev: SAPIV2Event): void {
  ...
  switch (sport) {
    case "football":
      cache = footballLiveV2Cache;
      break;
    case "basketball":
      cache = basketballLiveV2Cache;
      break;
    case "hockey":
      cache = hockeyLiveV2Cache;
      break;
    case "baseball":
      cache = baseballLiveV2Cache;
      break;
    case "tennis":
      cache = tennisLiveV2Cache;
      break;
  }
  if (!cache) {
...
function v1ScoreNum(
  s: number | { current?: number } | undefined,
): number | undefined {
  if (s === undefined || s === null) return undefined;
  return typeof s === "number" ? s : s.current;
}

function v1StatusStr(
  s: string | { description?: string } | undefined,
): string | undefined {
...
function applyV1ScorePatch(sport: SportKey, ev: V1ScoreEvent): void {
  ...
  if (sport === "tennis") {
    ...
    const mn = ev.minute;
    tennisV1WsScore.set(String(ev.id), {
      home: hs,
      away: as,
      status: st,
      minute: mn,
  ...
  broadcastMatchDelta(liveMatchIdForSportV2(sport, ev.id), {
    homeScore:
      homeScore ??
      (typeof existing.homeScore === "number" ? existing.homeScore : 0),
    awayScore:
      awayScore ??
      (typeof existing.awayScore === "number" ? existing.awayScore : 0),
    status: status,
    minute: ev.minute,
  });
}
...
function applyV1WsMessage(sport: SportKey, raw: unknown): void {
  ...
  if (Array.isArray(msg["events"])) {
    for (const ev of msg["events"] as V1ScoreEvent[])
      applyV1ScorePatch(sport, ev);
  } else if (msg["event"] && typeof msg["event"] === "object") {
    applyV1ScorePatch(sport, msg["event"] as V1ScoreEvent);
  } else if (Array.isArray(msg["data"])) {
    for (const ev of msg["data"] as V1ScoreEvent[])
      applyV1ScorePatch(sport, ev);
...
function connectV1SportWS(sport: SportKey): void {
  ...
  ws.addEventListener("close", () => {
    ...
    logger.warn(
      { sport, version: "v1", retryMs: delay },
      "V1 WS closed — scheduling reconnect",
    );
...
async function getFootballTodayV2(): Promise<SAPIV2Event[]> {
  const now = Date.now();
  if (footballTodayV2Cache && now - footballTodayV2FetchedAt < TODAY_V2_TTL)
    return footballTodayV2Cache;
  try {
    const resp = await fetch(`${SAPI_V2_FOOTBALL}/today`, {
      signal: AbortSignal.timeout(3000),
      headers: sapiHeaders(),
    });
...
async function getBasketballTodayV2(): Promise<SAPIV2Event[]> {
  const now = Date.now();
  if (basketballTodayV2Cache && now - basketballTodayV2FetchedAt < TODAY_V2_TTL)
    return basketballTodayV2Cache;
  try {
    const resp = await fetch(`${SAPI_V2_BASKETBALL}/today`, {
      signal: AbortSignal.timeout(3000),
      headers: sapiHeaders(),
    });
...
async function getHockeyTodayV2(): Promise<SAPIV2Event[]> {
  const now = Date.now();
  if (hockeyTodayV2Cache && now - hockeyTodayV2FetchedAt < TODAY_V2_TTL)
    return hockeyTodayV2Cache;
  try {
    const resp = await fetch(`${SAPI_V2_HOCKEY}/today`, {
      signal: AbortSignal.timeout(3000),
      headers: sapiHeaders(),
    });
...
async function getBaseballTodayV2(): Promise<SAPIV2Event[]> {
  const now = Date.now();
  if (baseballTodayV2Cache && now - baseballTodayV2FetchedAt < TODAY_V2_TTL)
    return baseballTodayV2Cache;
  try {
    const resp = await fetch(`${SAPI_V2_BASEBALL}/today`, {
      signal: AbortSignal.timeout(3000),
      headers: sapiHeaders(),
    });
...
export async function scanV2AllSportsForFinished(): Promise<void> {
  try {
    ...
    const bundles: SportBundle[] = [
      {
        prefix: ["football-v2"],
        events: footR.status === "fulfilled" ? footR.value : [],
        isFootball: true,
      },
      {
        prefix: ["bball-v2"],
        events: bballR.status === "fulfilled" ? bballR.value : [],
      },
      {
        prefix: ["hockey-v2"],
        events: hockR.status === "fulfilled" ? hockR.value : [],
      },
      {
        prefix: ["baseball-v2", "mlb-v2"],
        events: baseR.status === "fulfilled" ? baseR.value : [],
      },
      {
        prefix: ["tennis-v2"],
        events: tennR.status === "fulfilled" ? tennR.value : [],
      },
    ];

    for (const { prefix, events, isFootball } of bundles) {
      const sport: SportKey = isFootball
        ? "football"
        : prefix.includes("bball-v2")
          ? "basketball"
          : prefix.includes("hockey-v2")
            ? "hockey"
            : prefix.includes("tennis-v2")
              ? "tennis"
              : "baseball";
      for (const ev of events) {
        const st = ev.status as Record<string, unknown> | null | undefined;
        ...
        const htHome = isFootball
          ? typeof hS?.["period1"] === "number"
            ? (hS["period1"] as number)
            : undefined
          : undefined;
        const htAway = isFootball
          ? typeof aS?.["period1"] === "number"
            ? (aS["period1"] as number)
            : undefined
          : undefined;
        ...
        for (const p of prefix) {
          ...
          if (!finishedMatchResults.has(id))
            finishedMatchResults.set(id, record);
          await enqueueMatchSettlement({
            matchId: id,
            jobId: buildMatchSettlementJobId({
              matchId: id,
...
const TENNIS_LIVE_STATUSES = new Set([
  "Set 1",
  "Set 2",
  "Set 3",
  "Set 4",
...
function buildTennisLiveMatches(
  ...
  const sorted = [...tournaments].sort(
    (a, b) => tennisLeaguePriority(a.name) - tennisLeaguePriority(b.name),
  );
  ...
  for (const t of sorted) {
    ...
    for (const m of matches) {
      ...
      for (const sf of ["s1", "s2", "s3", "s4", "s5"] as const) {
        ...
        const a = p1[sf];
        if (h !== "" && a !== "")
          sets.push([parseFloat(h) || 0, parseFloat(a) || 0]);
      }
      ...
      const serveFromMatch = (() => {
        const raw =
          (m as any)?.serve ?? (m as any)?.server ?? (m as any)?.serving;
        const s = String(raw ?? "")
          .trim()
          .toLowerCase();
      ...
      const p0Serving = serveFromMatch
        ? serveFromMatch[0]
        : isServeTrue(getServeVal(p0));
      const p1Serving = serveFromMatch
        ? serveFromMatch[1]
        : isServeTrue(getServeVal(p1));
      ...
      const currentSetHomeWinProb = Math.min(
        0.97,
        Math.max(
          0.03,
          0.5 + (liveHomeP - 0.5) * 0.55 + (curSetH - curSetA) * 0.055,
        ),
      );
      const pGameHomeWin = estimateTennisLiveGameWinProb(
        liveHomeP,
        currentPoints,
        serving,
      );
      ...
      const homeSetScoreMomentum = Math.max(0, completedSetMomentum);
      const awaySetScoreMomentum = Math.max(0, -completedSetMomentum);
      const setExactScore = computeSetExactScoreOdds(
        curSetH,
        curSetA,
        currentSetHomeWinProb,
        0.22,
      ...
      const liveTennisExtras = computeLiveTennisExtras(
        liveHomeP,
        sets,
        homeScore,
        awayScore,
      ...
      let marketSuspension: Record<string, number> | undefined =
        existing?.marketSuspension
          ? Object.fromEntries(
              Object.entries(existing.marketSuspension).filter(([key, ts]) => {
                if (ts <= now) return false;
                if (
                  (key === "set2" || key === "set3") &&
                  ts > now + 60 * 60 * 1000 &&
                  settledSusp[key] === undefined &&
                  phaseHoldSusp[key] === undefined
                ) {
                  return false;
                }
                return true;
      ...
      if (
        Object.keys(settledSusp).length > 0 ||
        Object.keys(phaseHoldSusp).length > 0
      ) {
        marketSuspension = {
          ...(marketSuspension ?? {}),
          ...settledSusp,
      ...
      if (pointPlayed) {
        ...
        marketSuspension &&
        Object.keys(marketSuspension).length === 0
      ) {
        marketSuspension = undefined;
        suspensionReason = undefined;
      } else if (!pointPlayed && suspensionReason === "PONTO EM JOGO") {
        const active = marketSuspension
          ? Object.entries(marketSuspension).filter(
              ([k, ts]) => ts > now && settledSusp[k] === undefined,
            )
          : [];
      ...
      result.push({
        ...
        events: [],
        marketSuspension:
          marketSuspension && Object.keys(marketSuspension).length > 0
            ? marketSuspension
            : undefined,
...
function buildVolleyballLiveMatches(
  tournaments: VolleyTournament[],
): LiveMatchState[] {
  ...
  const sorted = [...tournaments].sort(
    (a, b) =>
      volleyballLeaguePriority(a.league) - volleyballLeaguePriority(b.league),
  );
  ...
  for (const t of sorted) {
    ...
    for (const m of matches) {
      ...
      const minsUntilStart = isNotStarted
        ? matchStartsInMinutes(m.date ?? "", m.time ?? "")
        : 0;
      ...
      const setNumFromScore =
        parseInt(home.totalscore || "0") + parseInt(away.totalscore || "0") + 1;
      const setNum = isLive
        ? isNumericStatus
          ? setNumFromScore
          : parseInt(m.status.split(" ")[1]!) || 1
        : apiLagging
          ? 1
          : 0;
      const statusLabel = isNotStarted
      ...
      result.push({
        ...
        _liveExtra: {
          vollSets,
          ...(isLive ? { currentPts: [ptH, ptA] as [number, number] } : {}),
        },
      });
    }
  }
...
function makeETMarketsFromScore(
  ...
  if (totalDiff > 0) {
    pNG_H = 0.45;
    pNG_A = 0.55;
  } // away trailing → more desperate
  else if (totalDiff < 0) {
    pNG_H = 0.55;
    pNG_A = 0.45;
  } // home trailing → more desperate
  else {
    pNG_H = 0.5;
    pNG_A = 0.5;
  }
...
function makePenMarketsFromScore(
  penHome: number,
  penAway: number,
): NonNullable<AdvancedMarkets["penExtra"]> {
  const marg = 0.07;
  const price = (p: number) =>
    mr(mc((1 / Math.max(0.01, p)) * (1 + marg), 1.01, 50));
...
async function buildLiveMatches(): Promise<LiveMatchState[]> {
  ...
  for (const league of sorted) {
    const ms: SAPIMatchV2[] = Array.isArray(league.match)
      ? league.match
      : [league.match];
  ...
  for (const league of sorted) {
    if (count >= 30) break;
    const matches: SAPIMatchV2[] = Array.isArray(league.match)
      ? league.match
      : [league.match];
    ...
    for (const m of matches) {
      ...
      const htStartedAt = isHT
        ? (existing?._htStartedAt ??
          (existing?.status === "HT" ? existing._htStartedAt : now))
        : undefined;
      if (
        isHT &&
      ...
      const minute = existing
        ? Math.max(existing.minute, minuteRaw)
        : minuteRaw;
      const gameStatus = isHT
        ? "HT"
        : isET
          ? "ET"
          : parseInt(m.status) > 45
            ? "2nd half"
            : "1st half";
      ...
      if (
        ...
        if (existing.marketSuspension) {
          const active = Object.fromEntries(
            Object.entries(existing.marketSuspension).filter(
              ([, ts]) => ts > now,
            ),
          );
        ...
        const safeMarkets = filterLiveMarkets(
          existing.markets,
          homeScore,
          awayScore,
          gameStatus,
        );
        const safeBase = filterLiveMarkets(
          existing._baseMarkets ?? existing.markets,
          homeScore,
          awayScore,
          gameStatus,
        );
        ...
        // Inject ET/pen markets if match is in extra time / penalties (always refresh in ET)
        {
          ...
          if (inETU) {
            ...
            mkt = {
              ...mkt,
              etExtra: makeETMarketsFromScore(etH, etA, homeScore, awayScore),
            };
          }
          if (m.penalties && !mkt.penExtra) {
            mkt = {
              ...mkt,
              penExtra: makePenMarketsFromScore(
                m.penalties.home_pen,
                m.penalties.away_pen,
              ),
            };
          }
        ...
        if (isGoal) {
          ...
          matchMarketSuspension = Object.fromEntries(
            FOOTBALL_SUSP_KEYS.map((k) => [
              k,
              now + footballSuspensionDelayMs("goal", k),
            ]),
          ) as Record<string, number>;
        ...
        matchMarkets = filterLiveMarkets(
          matchMarkets,
          homeScore,
          awayScore,
          gameStatus,
          ...
          if (matchMarkets.htft) {
            ...
            if (htH !== null && htA !== null) {
              ...
              if (sum3 > 1e-9) {
                ...
                const [o1, o2, o3] = probsToDecimalOdds(normed, 1.1);
        ...
        // live 1X2 after score changes (e.g. Home=1.06 but DC homeOrDraw shows 2.89).
        {
          ...
          if (sum > 0 && matchOdds.draw > 0) {
            const nH = pH / sum,
              ...
              nA = pA / sum;
            const dc = (p: number) =>
              mr(mc(1 / Math.max(1e-9, p * 1.04), 1.01, 100));
            matchMarkets = {
              ...matchMarkets,
              doubleChance: {
                homeOrDraw: dc(nH + nD),
              ...
              drawNoBet: matchMarkets.drawNoBet
                ? {
                    home: mr(
                      mc(
                        1 /
                          Math.max(1e-9, (nH / Math.max(1e-9, nH + nA)) * 1.04),
                        1.01,
                        100,
                      ),
                    ),
                    away: mr(
                      mc(
                        1 /
                          Math.max(1e-9, (nA / Math.max(1e-9, nH + nA)) * 1.04),
                        1.01,
                        100,
                      ),
                    ),
      ...
      const events: LiveMatchState["events"] = [];
      if (m.events?.event) {
        const evArr = Array.isArray(m.events.event)
          ? m.events.event
          : [m.events.event];
        for (const ev of evArr) {
          events.push({
      ...
      if (!isET && !m.penalties && minute < 90) {
        ...
        const muH =
          totL *
          (hStr / Math.max(1e-9, hStr + aStr)) *
          1.08 *
          remFrac *
          Math.max(0.4, 1 - 0.12 * stateRcHome + 0.04 * stateRcAway);
        const muA =
          totL *
          (aStr / Math.max(1e-9, hStr + aStr)) *
          0.92 *
          remFrac *
        ...
        const homeScore =
          m.home.goals === "?" ? 0 : parseInt(m.home.goals) || 0;
        const awayScore =
          m.away.goals === "?" ? 0 : parseInt(m.away.goals) || 0;
        const scored = homeScore + awayScore;
        // Win to Nil & Clean Sheet — zeroed when opponent has already scored
        const pWTNH =
          awayScore === 0 ? mc(pHomeWin * Math.exp(-muA), 0.01, 0.85) : 0;
        const pWTNA =
          homeScore === 0 ? mc(pAwayWin * Math.exp(-muH), 0.01, 0.85) : 0;
        ...
        const [oeOddL, oeEvenL] = probsToDecimalOdds(
          [mc(pOddFin, 0.2, 0.8), mc(pEvenFin, 0.2, 0.8)],
          1.06,
        );
        ...
        const egBucketTotal = Object.values(egBuckets).reduce(
          (a, b) => a + b,
          0,
        );
        const egLiveOdds =
          egBucketTotal > 0
            ? Object.fromEntries(
                Object.entries(egBuckets).map(([k, p]) => [
                  k,
                  Math.max(
                    1.01,
                    +(1 / ((p / egBucketTotal) * 0.87)).toFixed(2),
                  ),
                ]),
              )
            : {};
        ...
        matchMarkets = {
          ...
          ...(oeOddL && oeEvenL
            ? { goalOddEven: { odd: oeOddL, even: oeEvenL } }
            : {}),
      ...
      if (!isHT && !isET && !m.penalties && minute > 45 && minute < 90) {
        ...
        const hStrB = pH0b + pD0b * 0.45;
        const aStrB = pA0b + pD0b * 0.45;
        const mu2HH =
          2.6 *
          (hStrB / Math.max(1e-9, hStrB + aStrB)) *
          1.08 *
          rem2HFrac *
          Math.max(0.4, 1 - 0.12 * stateRcHome + 0.04 * stateRcAway);
        const mu2HA =
          2.6 *
          (aStrB / Math.max(1e-9, hStrB + aStrB)) *
          0.92 *
          rem2HFrac *
          Math.max(0.4, 1 - 0.12 * stateRcAway + 0.04 * stateRcHome);
        // Infer 2H score from events (goals scored after minute 45 come from 2H)
        const htGoalsH = events.filter(
          (e) =>
            e.team === "home" &&
            e.type.toLowerCase().includes("goal") &&
            e.minute <= 45,
        ).length;
        const htGoalsA = events.filter(
          (e) =>
            e.team === "away" &&
            e.type.toLowerCase().includes("goal") &&
            e.minute <= 45,
        ...
        if (h2CSTotL > 0) {
          const h2CSO = probsToDecimalOdds(
            h2CSArr.map(([, p]) => mc(p / h2CSTotL, 0.005, 0.7)),
            1.14,
          );
          const h2CSnew: Record<string, number> = {};
          h2CSArr.forEach(([score], idx) => {
            h2CSnew[score] = h2CSO[idx]!;
          });
          h2CSnew["Outro"] = mr(
            mc(
              1 / mc(h2CSOtherL / Math.max(1e-9, h2CSTotL) / 1.14, 0.005, 0.6),
              1.01,
              500,
            ),
          );
      ...
      const dangerEvent = events.find((e) => {
        ...
        if (
          !VAR_DANGER_TOKENS.some((token) => t.includes(token.replace("_", "")))
        )
          return false;
        if (isGoalEvent && !t.includes("var") && !t.includes("penalty"))
          return false;
        return true;
      });
      if (dangerEvent) {
        ...
        const reasonKey = Object.keys(VAR_REASON_MAP).find((k: string) =>
          evType.includes(k),
        );
        const reason = reasonKey ? VAR_REASON_MAP[reasonKey] : "SUSPENSO";
        const isHighPriority =
          reason.includes("VAR") || reason.includes("PENÁL");
      ...
      const state: LiveMatchState = {
        ...
        _driftPhase: 0,
        _liveExtra:
          Object.keys(footballExtra).length > 0 ? footballExtra : undefined,
        _firstSeenAt: firstSeenAt,
        _htStartedAt: isHT ? htStartedAt : undefined,
...
const _tennisLiveOddsCache = new Map<
  number,
  { home: number; away: number; at: number }
>();
  ...
  for (const id of toFetch) {
    const request = (async () => {
      try {
        ...
        const ft = markets.find(
          (m) =>
            (m.marketName === "Full time" || m.marketId === 1) &&
            m.isLive === true,
        );
        if (!ft) return;
        const homeChoice = (ft.choices ?? []).find((c) => c.name === "1");
        const awayChoice = (ft.choices ?? []).find((c) => c.name === "2");
        if (!homeChoice?.fractionalValue || !awayChoice?.fractionalValue)
          return;
...
const _tennisServingCache = new Map<
  number,
  { serving: [boolean, boolean]; at: number }
>();
...
const _tennisLiveDetailInFlight = new Map<
  number,
  Promise<TennisLiveDetailSnapshot | null>
>();

function parseTennisPointValue(
  p: string | undefined,
...
function tennisDisplayStatus(
  ...
  if (typeof statusCode === "number" && statusCode >= 13)
    return tennisSetLabel(statusCode - 12);
  return tennisSetLabel(Math.max(1, sets.length || 1));
}

async function getTennisMatchDetailV2(
  id: number,
): Promise<TennisLiveDetailSnapshot | null> {
  const now = Date.now();
  ...
  const p = (async () => {
    ...
    try {
      ...
      const points =
        hS?.point !== undefined && aS?.point !== undefined
          ? ([
              parseTennisPointValue(hS.point),
              parseTennisPointValue(aS.point),
      ...
      const serving =
        hs === null && as === null
          ? undefined
          : ([hs === true, as === true] as [boolean, boolean]);
      const statusCode =
        typeof ev.status === "object" && ev.status?.code !== undefined
          ? ev.status.code
          : ev.statusCode;
      const status = tennisDisplayStatus(
        v2StatusStr(ev.status),
        statusCode,
        sets,
      );
...
function isFootballV2LiveStatus(
  statusStr: string,
  statusCode?: number,
): boolean {
  const s = normalizeLiveStatus(statusStr);
...
const _v2StuckTracker = new Map<
  string,
  { minute: number; score: string; since: number }
>();
const FIRST_HALF_CLOCK_CAP_MIN = 59;
const SECOND_HALF_CLOCK_CAP_MIN = 109;

async function buildFootballLiveV2(
  events: SAPIV2Event[],
  ...
  const metaById = new Map<
    number,
    { countryRaw: string; countryKey: string; leagueName: string; prio: number }
  >();
  const primary: Array<{
    ev: SAPIV2Event;
    prio: number;
  ...
  for (const ev of events) {
    ...
    if (footballLeagueAllowedStrict(countryRaw, leagueName))
      primary.push({ ev, prio, fromUpcoming });
    else if (
      !isLeagueUniversallyBlocked(`${leagueName} ${homeTeam} ${awayTeam}`)
    )
      fallback.push({ ev, prio: prio === 999 ? 500 : prio, fromUpcoming });
  ...
  for (const ev of chosen) {
    ...
    const league =
      meta?.leagueName ?? normalizeLeagueName(v2TournName(ev.tournament), "");
    const { date, time } = v2EventDateTime(ev);
    const rc = v2FootballRedCardsCache.get(ev.id) ?? {
      home: 0,
      away: 0,
      fetchedAt: 0,
    };
    ...
    if (isHT) {
      ...
      if (kickoffSec > 0) {
        ...
        const minsFromKickoff = Math.floor(
          (now / 1000 - kickoffSec - clockSkewSec) / 60,
        );
        ...
        if (statusStr === "1st half" && !inferredSecondHalf) {
          ...
          minute = Math.min(
            FIRST_HALF_CLOCK_CAP_MIN,
            Math.max(0, minsFromKickoff),
          );
        } else if (statusStr === "2nd half" || inferredSecondHalf) {
          const shKickoff = existing?._liveExtra?.secondHalfKickoffSec;
          if (
            typeof shKickoff === "number" &&
            Number.isFinite(shKickoff) &&
            shKickoff > 0
          ) {
            const minsFromSecondHalf = Math.floor(
              (now / 1000 - shKickoff - clockSkewSec) / 60,
            );
            minute = Math.min(
              SECOND_HALF_CLOCK_CAP_MIN,
              Math.max(45, 45 + minsFromSecondHalf),
            );
          } else {
            minute = Math.min(
              SECOND_HALF_CLOCK_CAP_MIN,
              Math.max(45, minsFromKickoff - 15),
            );
          }
        } else {
          minute = Math.min(
        ...
        minute =
          statusStr === "1st half" ? 25 : statusStr === "2nd half" ? 70 : 45;
      }
    }
    const inferredHT =
      isBreak &&
    ...
    const shKickoffSec = isSecondHalf
      ? (existing?._liveExtra?.secondHalfKickoffSec ??
        (resolvedKickoffSec &&
        elapsedClockMinutes !== null &&
        elapsedClockMinutes >= 45
          ? Math.floor(now / 1000) - Math.max(0, minute - 45) * 60
          : 0))
      : 0;
    const baseSec = !resolvedKickoffSec
      ? null
      : newStatus === "HT"
        ? 45 * 60
        : isFirstHalf
          ? clamp(
              clockNowSec - resolvedKickoffSec - clockSkewSec,
              0,
              FIRST_HALF_CLOCK_CAP_MIN * 60,
            )
          : isSecondHalf
            ? clamp(
                45 * 60 +
    ...
    const clockStr =
      newStatus === "HT"
        ...
        : isFirstHalf
          ? minute > 45
            ? `45+${minute - 45}'`
            : `${String(Math.max(0, minute)).padStart(2, "0")}:${secStr}`
          : isSecondHalf
            ? minute > 90
              ? `90+${minute - 90}'`
              : `${String(Math.max(45, minute)).padStart(2, "0")}:${secStr}`
            : minute > 0
              ? `${minute}'`
              : undefined;
    ...
    if (existing) {
      ...
      const scored =
        homeScore !== existing.homeScore || awayScore !== existing.awayScore;
      const baseOdds = existing._baseOdds ?? {
        home: existing.odds.home,
        draw: existing.odds.draw,
        away: existing.odds.away,
      };
      ...
      const penGoals: [number, number] = isPen
        ? [
            Math.max(0, homeScore - penBase[0]),
      ...
      if (scored) {
        ...
        const filteredMarkets = withPen(
          filterLiveMarkets(existing.markets, homeScore, awayScore, newStatus),
        );
        const filteredBase = withPen(
          filterLiveMarkets(
            existing._baseMarkets ?? existing.markets,
            homeScore,
            awayScore,
        ...
        if (susp) {
          const active = Object.fromEntries(
            Object.entries(susp).filter(([, ts]) => ts > now),
          );
      ...
      const baseMarkets = filterLiveMarkets(
        rawMarkets,
        homeScore,
        awayScore,
        newStatus,
...
async function buildBasketballLiveV2(
  events: SAPIV2Event[],
): Promise<LiveMatchState[]> {
  const result: LiveMatchState[] = [];
  const now = Date.now();
  ...
  await Promise.all(
    events.slice(0, 40).map(async (ev) => {
      const idNum = Number(ev.id);
      if (!Number.isFinite(idNum) || idNum <= 0) return;
      const summary = await getBasketballIncidentSummaryV2(idNum).catch(
        () => null,
      );
  ...
  for (const ev of events) {
    ...
    if (hS && aS) {
      (
        [
          ...
          ["period3", "period3"],
          ["period4", "period4"],
        ] as Array<[keyof SAPIV2ScoreObj, keyof SAPIV2ScoreObj]>
      ).forEach(([hp, ap]) => {
    ...
    if (diff !== 0) {
      ...
      const loserOdds = Math.min(
        50,
        +(0.975 / Math.max(0.04, 1 - winPct)).toFixed(2),
      );
      liveOdds =
        diff > 0
    ...
    if (marketSuspension) {
      const kept = Object.fromEntries(
        Object.entries(marketSuspension).filter(([, ts]) => ts > now),
      );
    ...
    if (scoreChanged) {
      marketSuspension = Object.fromEntries(
        BBALL_SUSP_KEYS.map((k) => [k, now + 5_000]),
      );
      suspensionReason = "CESTA";
    } else if (statusChanged && (isHT || isPause)) {
      marketSuspension = Object.fromEntries(
        BBALL_SUSP_KEYS.map((k) => [k, now + 8_000]),
      );
      ...
      _liveExtra:
        quarters.length > 0 || (incidentSummary?.scoringEvents.length ?? 0) > 0
          ? {
              ...
              ...((incidentSummary?.scoringEvents.length ?? 0) > 0
                ? { basketballScoringEvents: incidentSummary!.scoringEvents }
                : {}),
            }
          : undefined,
    };
...
function buildHockeyLiveV2(events: SAPIV2Event[]): LiveMatchState[] {
  ...
  for (const ev of events) {
    ...
    if (hS && aS) {
      (
        ["period1", "period2", "period3", "period4"] as Array<
          keyof SAPIV2ScoreObj
        >
      ).forEach((p) => {
        const h = hS[p] as number | undefined;
    ...
    const isPause =
      code === 30 ||
      statusStr.toLowerCase().includes("intermission") ||
      statusStr.toLowerCase().includes("pause");
    const minute = isPause ? (pNum - 1) * 20 : (pNum - 1) * 20 + 10;
    const statusLabel = isPause
      ? `P${Math.min(pNum, 3)} Break`
      : code === 17
        ? "OT"
        : `P${Math.min(pNum, 3)}`;
    ...
    if (diff !== 0) {
      ...
      const loserOdds = Math.min(
        50,
        +(0.975 / Math.max(0.03, 1 - winPct)).toFixed(2),
      );
      liveOdds =
        diff > 0
          ? { home: winnerOdds, draw: 0, away: loserOdds }
    ...
    if (scoreChanged) {
      marketSuspension = Object.fromEntries(
        HOCKEY_SUSP_KEYS.map((k) => [k, now + 12_000]),
      );
      suspensionReason = "GOLO";
    } else if (statusChanged && isPause) {
      marketSuspension = Object.fromEntries(
        HOCKEY_SUSP_KEYS.map((k) => [k, now + 10_000]),
      );
      suspensionReason = "INTERVALO";
...
function buildBaseballLiveV2(events: SAPIV2Event[]): LiveMatchState[] {
  ...
  for (const ev of events) {
    ...
    const statusStr = v2StatusStr(ev.status);
    const statusType =
      typeof ev.status === "object" ? (ev.status as SAPIV2StatusObj).type : "";
    if (code === 100 || statusStr === "Ended" || statusStr === "Finished")
      continue;
    if (
      statusType !== "inprogress" &&
    ...
    if (scoreChanged) {
      marketSuspension = Object.fromEntries(
        BASEBALL_SUSP_KEYS.map((k) => [k, now + 12_000]),
      );
      suspensionReason = "RUN";
    } else if (inningChanged) {
      marketSuspension = Object.fromEntries(
        BASEBALL_SUSP_KEYS.map((k) => [k, now + 10_000]),
      );
...
function rememberFinishedTennisState(
  id: string,
  state: LiveMatchState,
  finishedAt: number,
): void {
...
function buildTennisLiveV2(events: SAPIV2Event[]): LiveMatchState[] {
  ...
  const sorted = [...events].sort(
    (a, b) => (v2StatusCode(b) ?? 0) - (v2StatusCode(a) ?? 0),
  );
  for (const ev of sorted) {
    ...
    const isMainTourLabel =
      v2LeagueLabel.startsWith("ATP") ||
      v2LeagueLabel.startsWith("WTA") ||
      v2LeagueLabel.startsWith("Grand Slam");
    const rawTournText = tennisTournamentSearchText(
      ev.tournament,
    ).toLowerCase();
    const isMainTourRaw =
      rawTournText.includes("wta") ||
      rawTournText.includes("atp") ||
      rawTournText.includes("grand slam") ||
      rawTournText.includes("wimbledon") ||
      rawTournText.includes("roland") ||
      rawTournText.includes("australian") ||
      rawTournText.includes("us open");
    ...
    const notLive =
      ...
      (code === 0 &&
        (sLow === "" ||
          sLow.includes("not start") ||
          sLow.includes("schedul"))) ||
      sLow.includes("ended") ||
      sLow.includes("finished") ||
      sLow.includes("complete") ||
      sLow.includes("not start") ||
      sLow.includes("postpone") ||
      sLow.includes("cancel") ||
      sLow.includes("walkover") ||
      sLow.includes("retired") ||
      sLow.includes("abandon") ||
      sLow.includes("awarded") ||
      sLow.includes("default");
    const matchAgeSeconds =
      ev.startTimestamp !== undefined ? nowSec - ev.startTimestamp : 0;
    ...
    const isZombie =
      ...
      (code === 8 ||
        ...
        code === 10 ||
        sLow.includes("set") ||
        sLow.includes("playing") ||
        sLow.includes("progress"));
    ...
    const serving =
      (ev.id ? _tennisServingCache.get(ev.id)?.serving : undefined) ??
      existing?._liveExtra?.serving;
    ...
    const v2HomeP =
      liveOdds.home > 0 && liveOdds.away > 0
        ? 1 / liveOdds.home / (1 / liveOdds.home + 1 / liveOdds.away)
        : 0.5;
    const v2TennisExtra = computeLiveTennisExtras(
      v2HomeP,
      sets,
      homeScore,
      awayScore,
      ...
      awayTeamId:
        typeof ev.awayTeam === "object" && ev.awayTeam?.id != null
          ...
          : ev.awayTeamId != null
            ? String(ev.awayTeamId)
            : undefined,
      league: tennisTournLabel(ev.tournament),
      country: v2TournCountry(ev),
      sport: "tennis",
      homeScore,
      awayScore,
...
const TENNIS_SIM_PLAYERS: Array<{
  home: string;
  away: string;
  tournament: string;
  country: string;
  ...
  return TENNIS_SIM_PLAYERS.map(({ home, away, tournament, country }, idx) => {
    ...
    if (diff !== 0) {
      ...
      liveOdds =
        diff > 0
          ? {
              home: Math.max(1.01, +(odds.home * (1 - factor)).toFixed(2)),
              draw: 0,
              away: Math.min(50, +(odds.away * (1 + factor)).toFixed(2)),
            }
          : {
              home: Math.min(50, +(odds.home * (1 + factor)).toFixed(2)),
              draw: 0,
              away: Math.max(1.01, +(odds.away * (1 - factor)).toFixed(2)),
            };
    }
...
async function buildTennisLiveV1(): Promise<LiveMatchState[]> {
  try {
    ...
    for (const g of games) {
      ...
      const compName = [g.competitionDisplayName, g.stageName, g.roundName]
        .filter(Boolean)
        .join(" ");
      ...
      const sets: [number, number][] =
        setSets.length > 0 ? setSets : [[homeScore, awayScore]];
      ...
      const gameStage = stages.find(
        (s) => s.name === "Game" || s.shortName === "Game",
      );
      ...
      const item: LiveMatchState = {
        ...
        markets: (() => {
          ...
          const pHome = mc(
            adjH + adjA > 0 ? adjH / (adjH + adjA) : 0.5,
            0.08,
            0.92,
          );
          return {
...
async function buildTennisLiveV1Cached(): Promise<LiveMatchState[]> {
  const now = Date.now();
  if (
    _tennisLiveV1StateCache &&
    now - _tennisLiveV1StateCache.fetchedAt < TENNIS_LIVE_V1_TTL
  )
  ...
  if (_tennisLiveV1StateCache) {
    if (!_tennisLiveV1StateInFlight) {
      _tennisLiveV1StateInFlight = buildTennisLiveV1()
        .then((matches) => {
          _tennisLiveV1StateCache = { matches, fetchedAt: Date.now() };
          return matches;
        })
        .finally(() => {
          _tennisLiveV1StateInFlight = null;
        });
    }
    return (
      (await Promise.race([
...
async function buildTennisUpcoming(): Promise<UpcomingMatch[]> {
  try {
    ...
    for (const g of games) {
      ...
      const pHome = mc(0.52 + (sr(1) - 0.5) * 0.2, 0.15, 0.85);
      ...
      const item: UpcomingMatch = {
        ...
        providerWinnerKnown:
          !!g.homeCompetitor?.isWinner || !!g.awayCompetitor?.isWinner,
        markets: {
          doubleChance: { homeOrDraw: 0, awayOrDraw: 0, homeOrAway: 0 },
          bothTeamsScore: { yes: 0, no: 0 },
          totalGoals: {
...
async function buildBasketballUpcoming(): Promise<UpcomingMatch[]> {
  try {
    ...
    const v1Games = await buildBasketballUpcomingV1().catch(
      () => [] as UpcomingMatch[],
    );
  ...
  try {
    ...
    for (const e of odds) {
      ...
      results.push({
        ...
        markets: makeVolleyballMarketsFromTeams(
          e.homeTeam.name,
          e.awayTeam.name,
          e.homeOdds,
          e.awayOdds,
...
function buildTennisUpcomingFromOddsEntries(
  odds: TennisOddsEntry[],
): UpcomingMatch[] {
  const seen = new Set<string>();
  ...
  for (const entry of odds) {
    ...
    out.push({
      ...
      markets:
        (entry.markets as AdvancedMarkets) ??
        makeAdvancedMarketsFromTeams(home, away),
    });
  }
  return out;
...
function mergeTennisUpcomingSources(
  base: UpcomingMatch[],
  oddsBased: UpcomingMatch[],
): UpcomingMatch[] {
  ...
  for (const m of oddsBased) {
    const pk = [_normTennisName(m.home), _normTennisName(m.away)]
      .sort()
      .join("|");
...
function hasRecentUpcomingFootballEligibility(
...
function hasRecentUpcomingEligibility(
  sport: SportKey,
...
function playerOdds(stat: number, appearances: number): number {
  ...
  const fair = 1 / rate;
  return Math.min(40.0, Math.max(1.1, Math.round(fair * 0.975 * 100) / 100));
}

function priceProbability(probability: number, cap = 80): number {
  if (!Number.isFinite(probability) || probability <= 0 || probability >= 0.985)
    return 0;
  return Math.min(
    cap,
    Math.max(1.1, Math.round((1 / probability) * 0.975 * 100) / 100),
  );
}
...
function normalizeTeamName(name: string): string {
  return name
    ...
    .replace(
      /\b(fc|afc|sc|cf|sv|ac|rsc|fk|sk|vfb|vfl|bsc|tsg|rb|cd|ud|sd|ca|ad|as|ss|us|ssc|sl|sfc|bfc|jfc|kfc|nk|gd|desportivo|futebol clube|clube)\b/g,
      "",
    )
    .replace(/[^a-z0-9 ]/g, "")
    .replace(/\s+/g, " ")
...
function findMatchingTeam(
  teams: FootballLeagueStatsTeam[],
  targetName: string,
): FootballLeagueStatsTeam | undefined {
...
function playerNameAliases(name: string): string[] {
  ...
  if (parts.length >= 3) {
    aliases.add(
      `${parts[0].slice(0, 1)} ${parts[1].slice(0, 1)} ${parts[parts.length - 1]}`,
    );
  }
...
function addProviderPlayerChoice(
...
function matchProviderOddForPlayer(
  target: Map<string, number>,
  playerName: string,
): number {
  ...
  for (const [choiceName, odds] of target.entries()) {
    if (
      aliases.some(
        (alias) => choiceName.includes(alias) || alias.includes(choiceName),
      )
    ) {
...
function normalizeProviderPlayerChoiceName(rawName: string): string {
  return String(rawName ?? "")
    .replace(
      /\b(over|under|mais de|menos de|to record|to have|at least|higher than|lower than)\b/gi,
      " ",
    )
    .replace(/\b\d+(?:\.\d+)?\+?\b/g, " ")
    .replace(/[()[\]{}|:/\\-]+/g, " ")
...
function extractPlayerNameFromLineMarketTitle(
...
function addProviderPlayerLineChoice(
  ...
  const side =
    parseProviderLineSide(rawChoiceName) ??
    parseProviderLineSide(rawMarketName);
  if (!side) return;
  const cleanName = normalizeProviderPlayerChoiceName(rawChoiceName);
  const fallbackName = extractPlayerNameFromExtendedLineMarketTitle(
    rawMarketName,
    marketKey,
  );
...
function matchProviderLinesForPlayer(
  ...
  for (const match of matches) {
    ...
    if (match.side === "over") {
      if (
        !existing.over ||
        Math.abs(match.odds - 1.85) < Math.abs(existing.over - 1.85)
      )
        existing.over = match.odds;
    } else {
      if (
        !existing.under ||
        Math.abs(match.odds - 1.85) < Math.abs(existing.under - 1.85)
      )
        existing.under = match.odds;
    }
...
function scorerMarketBucket(
  rawMarketName: string,
): keyof ProviderPlayerOddsBucket | null {
  ...
  if (
    ...
    return parseProviderLineValue(rawMarketName) != null
      ? "assistLines"
      : "assist";
  if (
  ...
  if (market.includes("tackles") || market.includes("desarmes"))
    return "tackles";
  return null;
}

const footballProviderPlayerOddsCache = new Map<
  string,
  { data: ProviderPlayerOddsBucket; fetchedAt: number }
>();
...
async function getFootballProviderPlayerOdds(
  matchId: string,
): Promise<ProviderPlayerOddsBucket> {
  ...
  if (
    cached &&
    Date.now() - cached.fetchedAt < FOOTBALL_PROVIDER_PLAYER_ODDS_TTL
  ) {
    return cached.data;
  }
  ...
  try {
    const resp = await fetch(
      `${SAPI_V2_FOOTBALL}/match/${providerMatchId}/odds/all`,
      {
        signal: AbortSignal.timeout(5000),
        headers: sapiHeaders(),
      },
    ...
    for (const market of data.data?.markets ?? []) {
      ...
      for (const choice of market.choices ?? []) {
        ...
        if (
          ...
          addProviderPlayerLineChoice(
            bucket[key],
            market.marketName ?? "",
            key,
            choiceName,
            line ?? parseProviderLineValue(choiceName),
            odds,
          );
        } else {
    ...
    footballProviderPlayerOddsCache.set(cacheKey, {
      data: bucket,
      fetchedAt: Date.now(),
    });
    return bucket;
...
function buildTeamMarkets(
  ...
  const byOdds = (a: PlayerMarketEntry, b: PlayerMarketEntry) =>
    a.odds - b.odds;
  ...
  for (const p of team.players) {
    ...
    const anytimeOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.anytime, p.name)
      : 0;
    const firstOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.first, p.name)
      : 0;
    const lastOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.last, p.name)
      : 0;
    const twoPlusOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.twoPlus, p.name)
      : 0;
    const hatTrickOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.hatTrick, p.name)
      : 0;
    const notScoreOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.notScore, p.name)
      : 0;
    const scoringBaseOdds = anytimeOdds > 0 ? anytimeOdds : derivedScorerOdds;

    if (firstOdds > 0)
      firstScorers.push({ ...base, stat: p.goals, odds: firstOdds });
    if (lastOdds > 0)
      lastScorers.push({ ...base, stat: p.goals, odds: lastOdds });
    if (twoPlusOdds > 0)
      twoPlusGoals.push({ ...base, stat: p.goals, odds: twoPlusOdds });
    if (hatTrickOdds > 0)
      hatTricks.push({ ...base, stat: p.goals, odds: hatTrickOdds });
    if (notScoreOdds > 0) {
      notToScore.push({
        ...base,
        stat: Math.max(0, p.appearances - Math.min(p.goals, p.appearances)),
        odds: notScoreOdds,
      });
    ...
    if (scoringBaseOdds > 0) {
      ...
      const fhOdds = Math.min(
        50,
        Math.round(scoringBaseOdds * 2.38 * 100) / 100,
      );
      firstHalfScorers.push({ ...base, stat: p.goals, odds: fhOdds });
      // ~58% of goals scored in 2H → fair odds ≈ anytime × 1.72
      const shOdds = Math.min(
        50,
        Math.round(scoringBaseOdds * 1.72 * 100) / 100,
      );
    ...
    if (p.goals >= 1 && p.assists >= 1) {
      ...
      if (saBase > 0) {
        ...
        const saOdds = Math.min(
          60,
          Math.round(Math.max(scoringBaseOdds * 2.0, saBase * 1.5) * 100) / 100,
        );
    ...
    const assistOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.assist, p.name)
      : 0;
    const assistBaseOdds = assistOdds > 0 ? assistOdds : derivedAssistOdds;
    if (assistBaseOdds > 0)
      assists.push({ ...base, stat: p.assists, odds: assistBaseOdds });

    const providerAssistLines = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.assistLines, p.name)
      : [];
    if (providerAssistLines.length > 0) {
      for (const item of providerAssistLines) {
        if (item.over)
          assistLines.push({
            ...base,
            id: `${p.id}:assists:${item.line}:over`,
            stat: p.assists,
            odds: item.over,
            line: item.line,
            side: "over",
        ...
        if (item.under)
          assistLines.push({
            ...base,
            id: `${p.id}:assists:${item.line}:under`,
            stat: p.assists,
            odds: item.under,
            line: item.line,
            side: "under",
    ...
    const providerShots = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.shots, p.name)
      : [];
    if (providerShots.length > 0) {
      ...
      const shotLine = pickPlayerStatLine(
        p.shotsTotal / Math.max(1, p.appearances),
        [0.5, 1.5, 2.5, 3.5, 4.5],
      );
      if (shotLine != null) {
    ...
    const providerShotsOnTarget = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.shotsOnTarget, p.name)
      : [];
    if (providerShotsOnTarget.length > 0) {
      ...
      const shotOnTargetLine = pickPlayerStatLine(
        p.shotsOn / Math.max(1, p.appearances),
        [0.5, 1.5, 2.5],
      );
      if (shotOnTargetLine != null) {
        const shotOnTargetOdds = playerLineOdds(
          p.shotsOn,
          p.appearances,
          shotOnTargetLine,
        );
        if (shotOnTargetOdds > 0)
          shotsOnTarget.push({
            ...base,
            id: `${p.id}:sot:${shotOnTargetLine}:over`,
            stat: p.shotsOn,
            odds: shotOnTargetOdds,
            line: shotOnTargetLine,
            side: "over",
          });
      }
    }

    const providerPasses = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.passes, p.name)
      : [];
    if (providerPasses.length > 0) {
      for (const item of providerPasses) {
        ...
        if (item.under)
          passes.push({
            ...base,
            id: `${p.id}:passes:${item.line}:under`,
            stat: p.passAttempts,
            odds: item.under,
            line: item.line,
            side: "under",
      ...
      const passesLine = pickPlayerStatLine(
        p.passAttempts / Math.max(1, p.appearances),
        [9.5, 19.5, 29.5, 39.5, 49.5, 59.5, 69.5],
      );
      if (passesLine != null) {
        const passesOdds = playerLineOdds(
          p.passAttempts,
          p.appearances,
          passesLine,
        );
        if (passesOdds > 0)
          passes.push({
            ...base,
            id: `${p.id}:passes:${passesLine}:over`,
            stat: p.passAttempts,
            odds: passesOdds,
            line: passesLine,
            side: "over",
    ...
    const providerTackles = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.tackles, p.name)
      : [];
    if (providerTackles.length > 0) {
      for (const item of providerTackles) {
        if (item.over)
          tackles.push({
            ...base,
            id: `${p.id}:tackles:${item.line}:over`,
            stat: p.tackles,
            odds: item.over,
            line: item.line,
            side: "over",
        ...
        if (item.under)
          tackles.push({
            ...base,
            id: `${p.id}:tackles:${item.line}:under`,
            stat: p.tackles,
            odds: item.under,
            line: item.line,
            side: "under",
      ...
      const tacklesLine = pickPlayerStatLine(
        p.tackles / Math.max(1, p.appearances),
        [0.5, 1.5, 2.5, 3.5, 4.5],
      );
      if (tacklesLine != null) {
        const tacklesOdds = playerLineOdds(
          p.tackles,
          p.appearances,
          tacklesLine,
        );
        if (tacklesOdds > 0)
          tackles.push({
            ...base,
            id: `${p.id}:tackles:${tacklesLine}:over`,
            stat: p.tackles,
            odds: tacklesOdds,
            line: tacklesLine,
            side: "over",
    ...
    const providerBookingLines = providerOdds
      ? matchProviderLinesForPlayer(providerOdds.bookingLines, p.name)
      : [];
    if (providerBookingLines.length > 0) {
      for (const item of providerBookingLines) {
        if (item.over)
          bookingLines.push({
            ...base,
            id: `${p.id}:bookings:${item.line}:over`,
            stat: cards,
            odds: item.over,
            line: item.line,
            side: "over",
        ...
        if (item.under)
          bookingLines.push({
            ...base,
            id: `${p.id}:bookings:${item.line}:under`,
            stat: cards,
            odds: item.under,
            line: item.line,
            side: "under",
    ...
    const sentOffOdds = providerOdds
      ? matchProviderOddForPlayer(providerOdds.redCard, p.name)
      : 0;
    if (sentOffOdds > 0) {
      redCards.push({ ...base, stat: dismissals, odds: sentOffOdds });
    } else {
      const derivedSentOffOdds = playerOdds(dismissals, p.appearances);
      if (derivedSentOffOdds > 0)
        redCards.push({ ...base, stat: dismissals, odds: derivedSentOffOdds });
    }
  }
...
function buildTeamLineupEligibility(
  lineup: { starters: LineupPlayerV2[]; bench: LineupPlayerV2[] } | undefined,
): TeamLineupEligibility | null {
  if (!lineup) return null;
  const starters = new Set<string>();
  const bench = new Set<string>();
  for (const player of lineup.starters ?? [])
    starters.add(normalizePersonName(player.name));
  for (const player of lineup.bench ?? [])
    bench.add(normalizePersonName(player.name));
  if (starters.size === 0 && bench.size === 0) return null;
  return { starters, bench };
}

function playerAllowedByLineup(
  playerName: string,
  eligibility: TeamLineupEligibility | null | undefined,
): boolean {
  if (!eligibility) return true;
  const aliases = playerNameAliases(playerName);
  if (aliases.length === 0) return false;
  return aliases.some(
    (alias) => eligibility.starters.has(alias) || eligibility.bench.has(alias),
  );
}

async function getFootballV2Lineups(
  matchId: string,
): Promise<LineupsResponseV2 | null> {
  const providerMatchId = sanitizeProviderMatchId(matchId);
  ...
  try {
    const resp = await fetch(
      `${SAPI_V2_FOOTBALL}/match/${providerMatchId}/lineups`,
      {
        signal: AbortSignal.timeout(8000),
        headers: sapiHeaders(),
      },
...
const v2Match1x2Cache = new Map<
  string,
  { odds: { home: number; draw: number; away: number }; fetchedAt: number }
>();
const V2_MATCH_1X2_TTL = 2 * 60_000;

async function getV2Match1x2Odds(
  sport: string,
  matchId: string,
  ...
  if (cached && Date.now() - cached.fetchedAt < V2_MATCH_1X2_TTL)
    return cached.odds;
...
function translateChoiceLabelPT(raw: string): string {
  ...
  if (/^over\s+/i.test(trimmed))
    return trimmed.replace(/^over\s+/i, "Mais de ");
  if (/^under\s+/i.test(trimmed))
    return trimmed.replace(/^under\s+/i, "Menos de ");
  return trimmed;
}
...
router.get(
  "/football-player-markets/:leagueId",
  async (req: Request, res: Response) => {
    const leagueId = String(req.params["leagueId"]);
    ...
    try {
      ...
      const providerOdds = matchId
        ? await getFootballProviderPlayerOdds(matchId)
        : null;
      ...
      res.json({
        ...
        country: meta.country,
        home: homeData
          ? buildTeamMarkets(homeData, providerOdds, homeEligibility)
          : null,
        away: awayData
          ? buildTeamMarkets(awayData, providerOdds, awayEligibility)
          : null,
      });
    } catch {
