...
/** GET /events/?status=live — real, dedicated live-events list (confirmed
 * real 2026-09-11). Used only to find a live football fixture's event id
 * to match against a live GOAL API fixture — see matchSync.ts. */
export async function getBzzoiroLiveEvents(): Promise<BzzoiroEvent[]> {
  if (!CONFIG.BZZOIRO_API_KEY) return [];
  try {
    const resp = await rawGet<BzzoiroEventsListResponse>("/events/", { status: "live", limit: 100 });
    return resp.results ?? [];
  } catch (err) {
